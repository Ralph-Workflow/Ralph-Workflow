...
- **No infrastructure-before-work rule:** Do not build, install, or configure anything before doing actual marketing work in this session. The infrastructure already exists. If something needs fixing, note it and fix it AFTER doing the work. Marketing first, engineering second.
- **Iterate or die rule:** If something produces zero results after a reasonable effort, change it. Don't optimize what's broken. Don't double down on a failing channel. Kill it and try something else. A marketer adapts; an engineer optimizes.
...
4. Check ALL channels — not just the one that had activity last session
5. Do marketing work first. Infrastructure fixes AFTER.
6. If I catch myself building automation, stop immediately and switch to conversations.
