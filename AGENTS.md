...
- **The only exception to lint-policy strictness is autogenerated code handled by the documented repository mechanism.** Do not invent ad hoc carve-outs.
...
- The only exception is documented autogenerated code handled by the repository's approved mechanism.
