...
#
# Remote directory: /tmp/rw-<sha256_16(pwd+hostname+server)>
# Lives in /tmp so the OS cleans it up automatically; no manual deletion needed.
# The hash makes the path unique per (local repo path, local machine, server)
# triple, so multiple machines, repos, or servers never collide on the same
# build server directory.
#
# Note: rw-build-server and rw-build-server-2 run Debian Linux (x86_64).
# cargo xtask verify produces no binary artifacts that need copy-back, so the
# macOS/Linux difference is transparent for the verification use case.
#
# Note: SSH non-interactive sessions on Debian may have a stripped PATH.
# Ensure ~/.bashrc on each build server sources ~/.cargo/env and adds
...
# Probe both build servers; probe.sh prints the selected server name and
# exports REMOTE_SERVER.  On failure, fall back to local execution.
REMOTE_SERVER=$("$SCRIPT_DIR/probe.sh") || {
  echo "[remote-build] no build server reachable, running locally" >&2
  exec "$@"
}
export REMOTE_SERVER

# Compute a stable, unique remote directory based on SHA-256 of
# local path + hostname + selected server.
...
echo "[remote-build] syncing to ${REMOTE_SERVER}:${REMOTE_ROOT}..." >&2
"$SCRIPT_DIR/sync.sh"
...
ssh "$REMOTE_SERVER" "cd $REMOTE_ROOT && \
...
ssh -t "$REMOTE_SERVER" "cd $REMOTE_ROOT && $*"
