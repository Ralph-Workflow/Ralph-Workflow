...
resubmit guidance (artifact type, submit tool, Markdown fallback path, prior-analysis
...
def _ra(phase: str, artifact_type: str, artifact_path: str) -> RequiredArtifact:
...
def test_hint_names_artifact_type_path_and_submit_tool() -> None:
    out = build_retry_hint(
        ...
        registry={"commit": _ra("commit", "commit_message", ".agent/artifacts/commit_message.md")},
        submit_tool_name="ralph_submit_md_artifact",
    )
    assert "commit_message" in out
    assert "ralph_submit_md_artifact" in out
    assert ".agent/artifacts/commit_message.md" in out
...
def test_hint_echoes_prior_analysis() -> None:
    out = build_retry_hint(
        "planning",
        "missing",
        registry={"planning": _ra("planning", "plan", ".agent/artifacts/plan.md")},
...
def test_hint_includes_example_document_when_given() -> None:
    out = build_retry_hint(
        "commit",
        "missing",
        registry={"commit": _ra("commit", "commit_message", ".agent/artifacts/commit_message.md")},
        submit_tool_name="ralph_submit_md_artifact",
        example_payload="---\ntype: commit_message\nsubject: fix(auth): reject expired token\n---",
    )
