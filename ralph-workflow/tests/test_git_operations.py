...
@pytest.mark.timeout_seconds(5)
def test_atomic_append_text_uses_unique_staging_filename_per_call(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Two same-process invocations with identical payload produce distinct staging paths.

    Pins the per-call-uniqueness contract for the ``_atomic_append_text`` staging
    sibling: the staging filename must include a per-call nonce (e.g. uuid4) so
    that two consecutive invocations against the same target with the same
    payload -- same process, same PID -- do not collide on the staging sibling.

    The previous sha256(payload) + os.getpid() derivation was NOT sufficient:
    both inputs are invariant across same-process, same-payload calls, so the
    staging path was reused and a concurrent invoker could either truncate a
    sibling's half-written content or hit ``FileExistsError`` on rename.

    The test intercepts ``Path.replace`` to capture each staging source path
    WITHOUT doing the real atomic publish, then asserts the two captured
    staging paths are distinct. ``Path.replace`` is the boundary where the
    staging sibling's filename first crosses into observable behavior -- if
    the staging file names collide, both calls would pass the SAME source to
    ``Path.replace``, which the test catches deterministically.
    """
    target = tmp_path / "shared_target.txt"
    target.write_bytes(b"initial-content\n")

    captured_sources: list[str] = []

    def _capturing_replace(
        self: Path,
        replacement: Path,
    ) -> Path:
        captured_sources.append(str(self))
        return original_replace(self, replacement)

    original_replace = Path.replace
    monkeypatch.setattr(Path, "replace", _capturing_replace)

    _atomic_append_text(target, "shared-payload\n", encoding="utf-8")
    _atomic_append_text(target, "shared-payload\n", encoding="utf-8")

    assert len(captured_sources) == 2, (
        f"Both invocations must reach Path.replace, captured {len(captured_sources)}: "
        f"{captured_sources!r}"
    )
    assert captured_sources[0] != captured_sources[1], (
        "Two same-process invocations with identical payload MUST produce "
        "distinct staging paths. Reused staging filename means a concurrent "
        "invocation can truncate or corrupt the sibling. "
        f"Got: {captured_sources!r}"
    )
