...
def test_idle_agent_with_untracked_file_catches_up_at_phase_boundary(
    tmp_git_repo: Path,
) -> None:
    """AC-04: three-agent convergence survives a stray untracked scratch file.

    The phase boundary is the ONLY seam that syncs an agent to another
    agent's landing *between* its own commits. An agent that is not
    committing right now -- running an analysis phase, say -- would
    otherwise never receive a sibling's work. Gating that seam on
    untracked files meant one scratch file silently disabled cross-agent
    synchronisation for the rest of the run, which is exactly the
    reported "auto rebase is not working" symptom.
    """
    main = _base_branch(tmp_git_repo)
    feature_a = tmp_git_repo.parent / "feature-idle-a"
    feature_b = tmp_git_repo.parent / "feature-idle-b"
    _add_worktree(tmp_git_repo, feature_a, "feature-idle-a")
    _add_worktree(tmp_git_repo, feature_b, "feature-idle-b")
    try:
        _commit(feature_a, "a.txt", "a\n", "feature a")
        first = auto_integrate_after_commit(
            _build_config(target=main), WorkspaceScope(feature_a), RebaseState()
        ...
        # Agent A is mid-phase and left a scratch file behind. It has
        # nothing to commit, so the commit seam will never fire for it.
        scratch = feature_a / "scratch.log"
        scratch.write_text("noise\n", encoding="utf-8")
        ...
        assert catch_up is not None, (
            "an untracked scratch file must not silently disable the "
            "cross-agent phase-boundary sync"
        )
        assert catch_up.fast_forwarded is True, (
            f"expected a landing, got action={catch_up.last_action!r} "
            f"reason={catch_up.last_reason!r}"
        )
        assert (feature_a / "b.txt").exists()
        assert scratch.read_text(encoding="utf-8") == "noise\n"
...
def test_resolution_survives_the_retry_after_a_concurrent_target_move(
    ...
    try:
        ...
        resolved = (feature / "shared.txt").read_text(encoding="utf-8")
        assert "<<<<<<<" not in resolved
        assert "feature+main" in resolved, (
            f"the agent's resolution was discarded by the retry: {resolved!r}"
        )
        assert "four" in resolved, (
            f"the sibling's concurrent line was discarded by the retry: {resolved!r}"
        )
        assert _run(feature, "rev-parse", "--verify", "MERGE_HEAD").returncode != 0
        assert len(resolver_calls) <= 2, (
            "the retry must not silently re-pay for a resolution it already "
