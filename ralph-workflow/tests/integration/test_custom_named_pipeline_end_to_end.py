...
class TestCustomNamedPipelineValidation:
    """Policy validation for the custom-named bundle."""
    ...
    def test_no_canonical_phase_names_in_policy(self, custom_bundle: PolicyBundle) -> None:
        """Custom phase names must not overlap with canonical default phase names."""
        phase_names = set(custom_bundle.pipeline.phases.keys())
        overlap = phase_names & CANONICAL_PHASE_NAMES
        assert overlap == set(), f"Custom phases overlap with canonical: {overlap}"


class TestCustomNamedPipelineHappyPath:
    """Full happy-path routing through the custom-named pipeline via reducer."""

    def test_happy_path_design_to_done(self, custom_bundle: PolicyBundle) -> None:
        """design → build → audit → sign_off → done happy path via reducer."""
        ...
        state, _ = reducer_reduce(state, PipelineEvent.AGENT_SUCCESS, policy)
        assert state.phase == "audit", f"Expected audit, got {state.phase}"

        state, _ = reducer_reduce(state, PipelineEvent.ANALYSIS_SUCCESS, policy)
        assert state.phase == "sign_off", f"Expected sign_off, got {state.phase}"

        state, _ = reducer_reduce(state, PipelineEvent.COMMIT_SUCCESS, policy)
        assert state.phase == "done", f"Expected done, got {state.phase}"
        assert state.get_outer_progress("cycles") == 1

    def test_analysis_loopback_routes_to_build(self, custom_bundle: PolicyBundle) -> None:
        """audit ANALYSIS_LOOPBACK sends back to build."""
        policy = custom_bundle.pipeline
        state = _initial_state()
        ...
        assert state.phase == "build", f"Expected build on loopback, got {state.phase}"
