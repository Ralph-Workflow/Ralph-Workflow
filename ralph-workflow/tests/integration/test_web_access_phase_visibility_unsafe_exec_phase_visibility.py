"""Phase visibility integration tests for unsafe_exec MCP tool.

Verifies that unsafe_exec is visible only in development and fix drains,
...
from ralph.mcp.tools.names import UNSAFE_EXEC_TOOL
...
class TestUnsafeExecPhaseVisibility:
    ...
    def test_unsafe_exec_is_visible_for_write_drain(self, drain: str) -> None:
        ...
        assert UNSAFE_EXEC_TOOL in tool_names, (
            f"unsafe_exec not in tools for drain {drain}; got: {sorted(tool_names)}"
        )

    @pytest.mark.parametrize("drain", [d.value for d in _NON_WRITE_DRAINS])
    def test_unsafe_exec_not_visible_for_non_write_drain(self, drain: str) -> None:
        session = _make_session(drain)
        registry = build_ralph_tool_registry(session, workspace=None, mcp_config=McpConfig())
        tool_names = {t.name for t in registry.list_definitions()}
        assert UNSAFE_EXEC_TOOL not in tool_names, (
            f"unsafe_exec unexpectedly visible for drain {drain}; got: {sorted(tool_names)}"
        )
