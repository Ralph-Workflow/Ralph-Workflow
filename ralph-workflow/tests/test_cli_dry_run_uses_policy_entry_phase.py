...
from ralph.cli.commands.run import print_dry_run
...
def test_dry_run_phase_text_uses_policy_entry_phase() -> None:
    """print_dry_run shows the policy entry_phase when no initial_state is provided."""
    console = _console()
    ctx = _display_context(console)
    bundle = _custom_policy_bundle(entry_phase="design")
    print_dry_run(None, _fake_config(), bundle, display_context=ctx)
...
def test_dry_run_phase_text_falls_back_to_unknown_without_policy() -> None:
    """print_dry_run shows 'unknown' when policy_bundle is None and no initial_state."""
    console = _console()
    ctx = _display_context(console)
    print_dry_run(None, _fake_config(), None, display_context=ctx)
    output = console.file.getvalue()
...
def test_dry_run_uses_initial_state_phase_when_present() -> None:
    """print_dry_run shows the initial_state.phase even when policy_bundle is provided."""
    console = _console()
    ctx = _display_context(console)
    bundle = _custom_policy_bundle(entry_phase="design")
    # Build a minimal PipelineState with phase="build"
    state = PipelineState(phase="build")
    print_dry_run(state, _fake_config(), bundle, display_context=ctx)
    output = console.file.getvalue()
