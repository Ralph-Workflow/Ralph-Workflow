...
def test_preview_payload_parser_coverage_matrix_uses_parser_emitted_shapes() -> None:
    ...
    mapped: tuple[tuple[str, str, dict[str, object]], ...] = (
        ...
        ("cursor", "edit_file", {"args": {"path": "a.py", "old_string": "x = 1", "new_string": "x = 2"}}),
        ("cursor", "edit_file", {"input": {"path": "a.py", "old_string": "x = 1", "new_string": "x = 2"}}),
        ("gemini", "Write", {"args": "{'file_path': 'a.py', 'content': 'x = 1'}"}),
        ("opencode", "read", {"input": {"path": "a.py", "content": "x = 1"}}),
        ("pi", "NotebookEdit", {"args": {"path": "a.ipynb", "kernel": "python", "source": "x = 1"}}),
    ...
    for parser, tool_name, metadata in mapped:
        ...
        assert build_edit_preview(tool_name, payload, width=80, terminal_bg_is_light=False) is not None, parser
    ...
    shipped = {path.stem for path in parser_dir.glob("*.py") if not path.stem.startswith("_")} - {
        "base", "agent_output_line", "text_accumulator", "interactive_transcript_event",
    }
...
def test_cursor_nested_args_envelope_reaches_preview_payload() -> None:
    ...
    event = {"type": "tool_call", "subtype": "started", "tool_call": {"edit_file": {"args": {"path": "a.py", "old_string": "x = 1", "new_string": "x = 2"}}}}
...
def test_notebook_edit_uses_the_kernel_lexer_despite_notebook_path() -> None:
    ...
    for kernel, source, lexer_name in (("python", "1\n", "PythonLexer"), ("julia", "f(x) = x + 1\n", "JuliaLexer")):
        payload = payload_from_tool_event("NotebookEdit", {"input": {"notebook_path": "analysis.ipynb", "kernel": kernel, "new_source": source}})
...
def test_bounded_previews_use_ascii_elision_and_one_multi_file_budget() -> None:
    ...
    preview = build_edit_preview("write_file", {"path": "a.py", "content": "\n".join(str(i) for i in range(41))}, width=80, terminal_bg_is_light=False, glyphs_enabled=False)
    output = io.StringIO()
    ...
    multi = build_edit_preview("read_multiple_files", {"content": payload}, width=80, terminal_bg_is_light=False)
    output = io.StringIO()
