...
def test_edit_preview_regression_many_hunks_stay_bounded() -> None:
    ...
    preview = build_edit_preview(
        "edit_file",
        {"path": "a.py", "edits": [{"oldText": f"old_{index} = 1", "newText": f"new_{index} = 2"} for index in range(200)]},
        width=100,
...
def test_edit_preview_regression_many_diff_hunks_stay_bounded() -> None:
    ...
    content = "\n".join(f"@@ -{index},10 +{index},10 @@\n" + "\n".join(f"+line_{line}" for line in range(10)) for index in range(1, 31))
    preview = build_edit_preview("git_diff", {"content": content}, width=100, terminal_bg_is_light=False)
    assert preview is not None
    output = _render(preview)
    assert len([line for line in output.splitlines() if line.strip()]) <= 45
    assert output.count("more lines") == 1
...
def test_overflowed_preview_cites_the_raw_overflow_log() -> None:
    ...
    preview = build_edit_preview("write_file", {"path": "a.py", "content": body}, width=100, terminal_bg_is_light=False, overflow_ref=overflow_ref)
    assert preview is not None
    assert _render(preview).count(f"[see {overflow_ref}]") == 1
    assert _render(preview).count("more lines") == 1
    record, _ = preview_record_text("write_file", {"input": {"path": "a.py", "content": body}}, overflow_ref=overflow_ref, glyphs_enabled=True)
    matches = [{"path": "a.py", "line": index, "text": f"line {index}"} for index in range(60)]
    grep_record, _ = preview_record_text("grep_files", {"input": {"content": json.dumps({"matches": matches})}}, overflow_ref=overflow_ref, glyphs_enabled=True)
    for projection in (record, grep_record):
        assert f"[see {overflow_ref}]" in projection
        assert "\x1b" not in projection
    unlinked_preview = build_edit_preview("write_file", {"path": "a.py", "content": body}, width=100, terminal_bg_is_light=False)
    assert unlinked_preview is not None
    unlinked_record, _ = preview_record_text("write_file", {"input": {"path": "a.py", "content": body}}, glyphs_enabled=True)
    unlinked_grep_record, _ = preview_record_text("grep_files", {"input": {"content": json.dumps({"matches": matches})}}, glyphs_enabled=True)
...
def test_edit_preview_regression_sanitizes_hostile_labels() -> None:
    ...
    preview = build_edit_preview("search_files", {"content": json.dumps({"matches": [hostile, {"path": hostile, "line": 1, "text": "x"}]})}, width=100, terminal_bg_is_light=False)
    assert preview is not None
    output = _render(preview)
