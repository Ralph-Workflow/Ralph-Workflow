...
# dark-terminal surface rather than the unknown-background fallback.
...
def _make_display(*, force_terminal: bool) -> tuple[ParallelDisplay, io.StringIO]:
    ...
    ctx = make_display_context(console=console, env={"CI": "1", "RALPH_TERMINAL_BG": "dark"})
...
def test_section_rule_uses_banner_border_style() -> None:
    ...
    assert _SKY_BLUE_24BIT_ANSI in rule_line, (
        f"section rule line must carry the dark theme.banner.border ANSI "
