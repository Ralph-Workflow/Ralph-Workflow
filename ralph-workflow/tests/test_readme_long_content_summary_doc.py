"""Regression tests: long-content display docs for streaming must stay in sync with code."""
...
_DOC_PATH = Path(__file__).resolve().parents[1] / "docs" / "sphinx" / "developer-internals.md"
...
def long_content_section() -> str:
    ...
    assert start != -1, (
        "developer-internals.md must contain a '## Streaming Blocks and "
        "Long-Content Display' heading"
    )
    end = text.find("\n## ", start + 1)
...
def test_readme_states_default_on(long_content_section: str) -> None:
    ...
    assert match is not None, (
        "developer-internals.md long-content section must state the summary "
        "is on/enabled by default"
    )
...
def test_readme_documents_200_char_inline_cap(long_content_section: str) -> None:
    assert "200" in long_content_section, (
        ...
        "200-character inline summary cap"
    )
    assert "120" in long_content_section, (
        "developer-internals.md long-content section must document the "
        "120-character streaming summary cap"
    )
...
def test_threshold_constant_matches_readme(long_content_section: str) -> None:
    assert str(SUMMARY_THRESHOLD) in long_content_section, (
        f"developer-internals.md must document the summary threshold "
        f"({SUMMARY_THRESHOLD}) to match "
        f"ralph.display.long_content_summary.SUMMARY_THRESHOLD"
    )


def test_section_is_non_empty(long_content_section: str) -> None:
    assert len(long_content_section) > _MIN_SECTION_LEN, (
        "developer-internals.md long-content section must have content"
    )
