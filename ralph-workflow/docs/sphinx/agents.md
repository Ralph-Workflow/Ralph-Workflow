...
Every headless invocation passes `--print-timeout 2h` immediately before
`--print`: AGY's internal five-minute print deadline otherwise terminates
long, healthy sessions around 298 s, and two hours intentionally exceeds
both Ralph Workflow's 95-minute generic session ceiling and the default
90-minute development timebox hard stop. An exit-0 run that still lacks the
