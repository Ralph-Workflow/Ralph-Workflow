...
## Conflict resolution ended while rebasing or merging

**Symptom:** The run output names `CONFLICT_INACTIVITY`, `OPERATOR_CAP_REACHED`, or `SUPERVISION_INFRASTRUCTURE_FAILURE` while resolving a merge or paused rebase.

**Cause and next action:**

- **`CONFLICT_INACTIVITY`** means no recognized activity arrived for the fixed `900`-second conflict-resolution window. The output includes the last activity kind and time, total attempt duration, and unresolved paths. Inspect that path list and the resolver transcript before retrying.
...
- **`SUPERVISION_INFRASTRUCTURE_FAILURE`** means the authenticated standalone-MCP activity relay could not deliver or acknowledge an MCP tool event. Ralph Workflow stops rather than falsely classifying the resolver as idle. Check the local MCP process log, repair the tool surface, and re-run; do not treat this as a conflict-resolution failure.
- **`TRANSPORT_LOOP_DETECTED`** means repeated failing transport activity was evidence that the attempt could not succeed. Ralph Workflow hands over through the configured chain without charging the conflict work; inspect the relay/tool logs before retrying.
- **No-progress observation** means activity continued without a recorded resolver advance. It is reported separately from a hang or conflict verdict; inspect unresolved paths and tool availability before deciding whether to retry.
- **Out-of-reach** means Ralph Workflow classified a binary, unreadable, or otherwise unsafe conflict before handing it to an agent. Choose and record a safe side or resolve the repository state manually, then retry.
- **Budget suppression** means the same durable identity already failed without relevant new information. Change the conflict inputs or resolve it manually; Ralph Workflow will not start another identical resolver attempt.

Ralph Workflow reaps the resolver and MCP process tree before it scans markers, continues a rebase, commits a merge, or hands a non-success result to the existing deterministic abort path. Landed rebase-stop SHAs and remaining conflict identity are persisted in `.ralph/conflict-resolution-progress.json`; abort does not drop already-landed stops, and the next attempt resumes at the first unlanded stop. Recovery uses the configured `rebase_conflict_resolution` chain's retry, backoff, and fallback behavior; add a second chain candidate if the load-time no-fallback warning applies.
...
**Fix:** Read the ending line for the typed reason, last activity vs last progress, candidates tried, and unresolved paths. If the chain has a single agent, add a fallback in `[agent_chains.rebase_conflict_resolution]` — load warns when there is no fallback. If output says the conflict budget is exhausted, Ralph Workflow has stopped starting rebases for that identity; a distinct remote conflict can still run because it uses `scope=remote`.
