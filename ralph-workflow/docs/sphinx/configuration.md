...
| `auto_integrate_enabled` | `true` | On by default: re-evaluated statelessly at every seam (startup, pre/post planning, before/after development, after commit, phase boundaries, workers, and background catch-up). Ralph Workflow rebases onto the current local target, falls back to endpoint merge, then fast-forwards the target. Equality/ancestor cases are fast-forward-or-noop outcomes. Only `auto_integrate_enabled = false` disables integration; set it to `false` for byte-identical manual git control. |
| `auto_integrate_target` | (auto-detect) | Shared integration branch name. When set (e.g. `"develop"`) it is used verbatim, provided that branch exists as a **local** branch. Resolution is a stateless local-refs-only check re-derived at every seam: a branch is never created from `refs/remotes/origin/<target>`, so remote state can never decide the base of a local rebase. When unset, the target is auto-detected: the `origin/HEAD` default-branch *name* when a local branch of that name exists, otherwise `main`, otherwise `master`. If no candidate exists locally the step skips with a recorded reason and never guesses. |
...
| `auto_integrate_push_enabled` | `false` | Off by default: when `false`, auto-integration is strictly local and the local mainline ref is never pushed to a remote. When `true`, EVERY successful advance of the target branch (the local rebase/merge/endpoint-merge/fast-forward landing the auto-integrate step performed) is also pushed to EVERY configured remote, best-effort per remote and fail-open. The push runs ONLY after the local landing already succeeded, a remote failure never fails the run, never reverts the local ref, and never changes the landing result. The push never force-pushes and never moves any branch the operator did not ask for. The summary is recorded on the run state and rendered in the `auto-integrate:` line, so a partial push (e.g. one remote down, one up) is operator-visible. |
| `auto_integrate_push_timeout_seconds` | `30.0` | Per-remote wall-clock budget for an auto-integrate push (must be `> 0` and `<= 300`). On timeout the remote is skipped and the outcome recorded; the run is never failed by an unreachable remote. Applied independently per remote, so a slow remote does not consume the budget of a fast one. |
| `auto_integrate_remote_sync_enabled` | `false` | **Off by default (opt-in).** When `false`, auto-integration is strictly local: no `git fetch`, no `git push`, and no remote-driven rebase or reconcile happens. Every existing Part-A integration contract is preserved byte-identically: setting this to `true` is the ONLY way to opt into the remote-sync tier. Setting it to `true` implies `auto_integrate_fetch_enabled = true` and emits a deprecation warning naming the replacement if the legacy `auto_integrate_push_enabled` key is also set. The throttled pull keeps `refs/heads/<target>` in step with `<auto_integrate_remote_target>/<target>`, the push publishes every successful landing to the configured remote, and a rejected push runs the bounded fetch -> rebase-target-onto-remote -> re-integrate -> push reconcile loop on a jittered exponential backoff. A run that finishes with an unpushed target can opt into a visible end-of-run waiting state via `auto_integrate_remote_wait_seconds > 0`. |
| `auto_integrate_remote_target` | `"origin"` | Configured remote name to sync the local target branch against (must be a non-empty string). Defaults to `"origin"`; any configured remote is accepted. When the named remote is not configured in this repository the step degrades to local-only integration with a recorded skip reason -- it never raises. The push side only ever touches this one remote; the deprecated legacy `auto_integrate_push_enabled = true` key used to fan out to every configured remote, and this flag replaces that fan-out with single-remote semantics. |
| `auto_integrate_remote_sync_interval_seconds` | `300.0` | Minimum wall-clock seconds (must be `>= 0`) between two PULLS for a given `(repo root, remote, target)` triple. `0` means every seam: the throttle never suppresses a healthy fetch. The push side is NOT throttled: it follows successful landings, which are already bounded by the seams. A failed fetch does NOT arm the next window; only a healthy fetch arms it, so a transient blip never buys a whole interval of unrefreshed probes. |
| `auto_integrate_remote_backoff_max_seconds` | `300.0` | Upper bound (must be `>= 0`) on the per-remote exponential, jittered backoff applied after consecutive remote-side failures, both between seams and inside the end-of-run waiting state. The base is `auto_integrate_remote_sync_interval_seconds`; consecutive failures widen the gap exponentially with jitter, capped at this ceiling. Any successful remote interaction resets the backoff to the base interval. The ceiling bounds the GAP between attempts, never the NUMBER of attempts -- a transient failure never stops a later seam from trying again. |
| `auto_integrate_remote_wait_seconds` | `0.0` | How long (must be `>= 0`) a FINISHED run may sit in the visible waiting-for-remote state when it still holds an unpushed target. `0` (default) means do not wait: preserve today's exit behavior verbatim -- record `"landed locally, not published to <remote>: <reason>"` and exit. Any positive value opts the run into waiting: retry the fetch -> reconcile -> push sequence on the exponential jittered backoff capped by `auto_integrate_remote_backoff_max_seconds`, until this cap is reached or the push succeeds. The waiting state is interruptible by the existing interrupt/exit-pause path and never blocks the pipeline while work remains. |
...
`refresh suppressed by throttle`, `no remote configured`, `fetch disabled`, plus
