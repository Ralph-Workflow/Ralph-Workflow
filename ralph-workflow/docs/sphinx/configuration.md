...
| `auto_integrate_push_enabled` | `false` | Deprecated alias for `auto_integrate_remote_sync_enabled`. When `true`, it enables fail-open sync with the one configured `auto_integrate_remote_target`; publishing happens only after local landing, never force-pushes, and never changes the local landing result. |
| `auto_integrate_push_timeout_seconds` | `30.0` | Wall-clock budget for the configured remote auto-integrate push (must be `> 0` and `<= 300`). On timeout the outcome is recorded and the run continues without changing local success. |
