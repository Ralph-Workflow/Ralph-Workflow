...
| `max_consecutive_resolver_attempts` | `8` | integer `>= 1` | Resolver attempts allowed for one unchanged conflict identity and one conflict-resolution strategy. Exhausting a strategy advances the persisted ladder rather than suppressing integration globally; a changed identity starts a fresh ladder. |
