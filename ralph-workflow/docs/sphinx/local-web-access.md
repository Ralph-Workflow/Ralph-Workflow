...
Ralph provides three distinct web capabilities — search, visit, and crawl — that are designed to be complementary rather than overlapping. Understanding what each does is essential for choosing the right tool.

## Search vs Visit vs Crawl

| Concept | Ralph Tool | What it Returns |
|---|---|---|
...
Ralph can **search** the web, **visit** a page directly, and, when needed, **delegate crawling** to a stronger local crawler.

## Native vs Upstream

### Native: `visit_url` (built-in)

The `visit_url` tool is Ralph's built-in page reader. It fetches a single URL and returns clean, extracted text without requiring any additional services.
...
**No setup required** — it ships with Ralph and works out of the box.

### Upstream: Crawl4AI / Firecrawl (local sidecar)

For multi-page crawls, JavaScript-rendered SPAs, or structured data extraction, Ralph can delegate to a local upstream MCP server.
...
- Ralph runs in an isolated container or VM with no internal service exposure
...
The canonical tool names exposed through Ralph's MCP surface are:
