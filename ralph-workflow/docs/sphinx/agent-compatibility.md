...
- **Caveats**:
    ...
    - The latest v1.1.8 manual live smoke created its requested file and showed parser/tool activity without a permission prompt, but did not durably submit `smoke_test_result` or write the completion sentinel. Ralph Workflow reports that failure explicitly; AGY is not yet proven end-to-end for artifact submission. Its plain-text parser reports observed output; stream-json is available as a separately observed CLI format.
