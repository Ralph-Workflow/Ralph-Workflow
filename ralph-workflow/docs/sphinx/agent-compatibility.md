...
- **Caveats**:
    ...
    - The latest v1.1.8 manual live smoke created its requested file, showed parser/tool activity without a permission prompt, and produced a valid fallback `smoke_test_result`. Ralph Workflow canonically promoted it, recorded the receipt and host completion sentinel, and exited successfully. Its plain-text parser reports observed output; stream-json is available as a separately observed CLI format.
