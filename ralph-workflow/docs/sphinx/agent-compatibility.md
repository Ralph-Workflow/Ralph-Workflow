...
- **Credentials**: Set `CURSOR_API_KEY` or run `agent login`. On macOS, Ralph Workflow accepts the OS-keychain entry created by `agent login` as the default unattended credential input; it also preserves file-backed non-MCP login material from `~/.cursor` in each invocation's private `HOME` while replacing `mcp.json` with the run-scoped endpoint.
...
- **Failure behavior**: If `CURSOR_API_KEY`, file-backed login material, and (on macOS) the `agent login` keychain entry are all absent, Ralph Workflow fails before launch with a `USER_CONFIG` error and the hint `agent login required`; it does not reset the session as stale or consume retry budget.
