...
| Global MCP config overlay with cross-process lock | transport module (e.g. `ralph/mcp/transport/agy.py`) | A run-scoped write+restore of a user-global config file must serialize INDEPENDENT processes, not just threads: `agy_workspace_mcp_endpoint` pairs its process-local `threading.Lock` with a bounded advisory `fcntl.flock`/`msvcrt.locking` sidecar (`_agy_config_overlay_lock`) that fails closed with a timeout error instead of racing a sibling process's restore. Reuse that pattern; never rely on a process-local lock for a file another process can write. |
