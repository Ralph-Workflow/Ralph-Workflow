...
| Compiled-in native MCP config | runtime resolver + `ralph/mcp/transport/private_config_root.py` | Build the merged native config once, publish it under an invocation-owned root with `prepare_private_config_root`, and point the child process at that root (`HOME`, `KIMI_CODE_HOME`, or the agent's equivalent). Never write or restore workspace or operator-global config for a live session: private roots let sibling processes overlap without locks, preserve distinct endpoints, and are removed by the runtime cleanup hook. |
