...
| `syntax_highlight` + `language` parser metadata | any parser's text-event metadata | The canonical event renderer (`ralph/display/agent_event_renderer.py`) consumes the annotation transport-neutrally: a fenced code body in the text is re-tokenized through the shared Pygments/theme seam (`ralph/display/_edit_preview_render.py: highlight_code_spans`) and rendered with real lexer-derived spans, `.plain` preserving the original content. Unknown languages and non-fenced text fall back to the flat body style. |
| Global MCP config overlay: cross-process lock + crash recovery | transport module (e.g. `ralph/mcp/transport/agy.py`), primitives in `ralph/mcp/transport/config_overlay.py` | A run-scoped write+restore of a user-global config file has two obligations, both met by the shared primitives — do not hand-roll either. (1) Serialize INDEPENDENT processes, not just threads: pair the process-local `threading.Lock` with `mcp_config_overlay_lock`, a bounded advisory `fcntl.flock`/`msvcrt.locking` sidecar that fails closed with a timeout error instead of racing a sibling process's restore. (2) Survive an abnormal exit: `stage_config_overlay` writes a durable `<name>.ralph-backup` record before the overwrite and `reclaim_config_overlay` — called FIRST, inside the lock — puts the operator's file back when a killed run never reached its `finally`. The reclaim restores only while the file on disk is still byte-identical to the overlay the record names, so a newer operator edit supersedes the record instead of being clobbered. A bare `try/finally` restore is not sufficient: `SIGKILL`, OOM, reboot and power loss all skip it. |
