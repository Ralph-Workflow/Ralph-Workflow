...
The NO_OUTPUT_AT_START trip is deferred (returns ``CONTINUE`` instead of ``FIRE``) when ANY of the following LIVE signals is present at the moment of the evaluate:

- ``classify_quiet()`` returns ``AgentExecutionState.WAITING_ON_CHILD`` -- the execution strategy has already classified the run as waiting on a live child. This early-exit is evaluated BEFORE the corroborator and cumulative gates so the FIRST ``WAITING_ON_CHILD`` entry (where ``cumulative_waiting_on_child_seconds == 0.0``) defers the 30s ``NO_OUTPUT_AT_START`` short kill and lets ``_handle_waiting_branch`` consult its cumulative ``CHILDREN_PERSIST_TOO_LONG`` ceiling (default 600s, computed via ``_effective_waiting_ceiling``) instead. This fixes the prompt false-positive where a subagent dispatched at invocation start caused ``NO_OUTPUT_AT_START`` to fire at 30s before the WAITING_ON_CHILD deferral path could apply. The 600s ceiling remains the correct upper bound for live-child stalls.
- ``self._safe_corroborate()`` returns a ``CorroborationSnapshot`` whose ``alive_by`` is a FRESH corroboration state (``FRESH_PROGRESS`` or ``FRESH_HEARTBEAT_ONLY``) -- the corroborator (process tree / OS descendant scan / heartbeat) confirms a live child agent with a recent progress or heartbeat signal. The deferral consults ``_safe_corroborate()`` LIVE inside ``_evaluate_no_output_at_start`` and gates on ``_alive_by_is_fresh(corroboration.alive_by)`` (NOT ``self._last_alive_by``, which is stale post-fire state set only by ``NO_PROGRESS_QUIET`` at ``idle_watchdog.py:620`` and is never populated for ``NO_OUTPUT_AT_START``). Stale ``AliveBy`` values (``OS_DESCENDANT_ONLY_STALE_PROGRESS``, ``CPU_IDLE_WHILE_ALIVE``, ``LOG_STALE_WHILE_ALIVE``, ``STALE_LABEL_ONLY``) and ``None`` DO NOT defer: they describe a child that has stopped producing fresh evidence and the short ``NO_OUTPUT_AT_START`` kill MUST still apply.
...
The deferral is consult-only -- the underlying NO_OUTPUT_AT_START trigger logic, the StuckClassifier gate, and the channel-evidence deferral are all unchanged. An agent that returns an empty ``CorroborationSnapshot`` AND has no prior waiting run AND is not classified as ``WAITING_ON_CHILD`` still fires ``NO_OUTPUT_AT_START`` at the threshold (no false-positive deferral). See ``tests/agents/idle_watchdog/test_no_output_at_start.py`` for the regression tests that pin the WAITING_ON_CHILD early-exit (``test_no_output_at_start_defers_on_first_waiting_on_child_entry``) and the 600s ceiling fallback (``test_no_output_at_start_fires_after_waiting_ceiling_reached``), and ``TestNoOutputAtStartLiveCorroborationDefer`` in ``tests/agents/test_idle_watchdog_no_output_at_start_lifecycle.py`` for the live-corroboration deferral contract.
...
A new ``_classify_generic_child_signal`` classifier in ``ralph/agents/execution_state/_helpers.py`` is wired into ``BaseExecutionStrategy.observe_line`` so EVERY transport's ``observe_line`` automatically invokes the watchdog's subagent activity sink on child-scope signals (no per-transport classifier needed for Claude, Codex, Generic, Agy, Nanocoder). The classifier is intentionally STRICT: it recognises ONLY event names that explicitly carry a child- or subagent-scope marker, plus a small set of plain-text markers, plus the ``child_`` / ``subagent_`` prefix family. Bare provider-level event names that appear in parent-level stdout (``progress``, ``tool_call``, ``task_progress``, ``heartbeat``, ``alive``) are NOT classified -- these are generic provider wire frames and do NOT prove that a subagent is active. The classifier recognises:
- JSON envelopes whose ``type`` / ``event`` key is in ``{child_progress, subagent_progress}`` -> ``CHILD_PROGRESS``. Bare ``progress`` / ``tool_call`` / ``task_progress`` do NOT classify.
- JSON envelopes whose ``type`` / ``event`` key is in ``{child_heartbeat, subagent_heartbeat, child_alive, subagent_alive}`` -> ``CHILD_HEARTBEAT``. Bare ``heartbeat`` / ``alive`` do NOT classify.
...
The OpenCode strategy continues to override ``observe_line`` entirely (it owns the specialised OpenCode wire format, which ALWAYS emits ``tool_call`` / ``heartbeat`` / ``progress`` events with a ``child_id`` field); the base implementation is only invoked for strategies that do NOT override ``observe_line`` (Claude, Codex, Generic, Agy, Nanocoder), so there is NO double-invocation. The OpenCode specialised classifier is intentionally PERMISSIVE (``_OPENCODE_CHILD_PROGRESS_TYPES`` includes bare ``progress`` / ``tool_call`` because the OpenCode wire format ALWAYS attaches a ``child_id`` field) and is unchanged. See ``tests/agents/execution_state/test_generic_child_signal.py`` for the contract (positive tests for child-scoped events, terminal-event and prose-line regression tests, plus the ``does_not_invoke_sink_on_bare_*`` regression tests that pin the no-false-positive contract for bare ``heartbeat`` / ``tool_call`` / ``progress`` / ``alive`` / ``task_progress`` frames in cross-transport strategies).
