...
def _fast_forward_target(
    ...
    empty when the fast-forward succeeded. The atomic guards:

    * ``is_ancestor(target, feature_sha)`` first (AC-08: never force).
    * If the target is checked out in another worktree and that
      worktree is clean: ``fast_forward_via_worktree`` (itself
      ff-guarded by ``git merge --ff-only``).
    * If the target is checked out in another worktree and that
      worktree is dirty: skip with a recorded reason (AC-09).
    * If the target is checked out nowhere: ``compare_and_swap_branch``
      with the freshly-observed target SHA. The CAS fails closed on
      a concurrent update (AC-08 TOCTOU closure).
    """
    # AC-08 guard: target must be an ancestor of feature_sha.
    if not is_ancestor(repo_root, target, feature_sha):
