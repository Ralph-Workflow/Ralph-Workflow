...
class GeneralConfig(RalphBaseModel):
    ...
    agent_workspace_change_weights: dict[str, float] = Field(
        ...
        description=(
            ...
            " / .venv / .agent/tmp / .agent/raw / .agent/ralph-explore / .agent/receipts"
            " / .agent/prompt_history / .agent/artifact-formats / .agent/logs"
            " / completion_seen_*.json / worker-unit tmp,logs,handoffs / root checkpoint.json"
            " / transport MCP overlays / every file directly under .agent/),"
