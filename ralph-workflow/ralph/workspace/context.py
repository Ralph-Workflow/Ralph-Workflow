...
- **Restore exactly.** Every exit path -- normal completion,
  ...
  drift in active-prompt bytes, effective config, policy bundle, agent
...
def workspace_context(target_root: Path) -> Iterator[WorkspaceContext]:
    ...
    """
    # Lazy imports: every dependency here -- config loader, policy
    # loader, agent registry, MCP plan resolver -- pulls in a chain that
    # ends in ``ralph.workspace`` being re-entered (the MCP plan resolver
    # depends on ``ralph.mcp.artifacts.product_spec``, which imports
    # ``ralph.workspace``). Importing them here, after the cyclic chain
    # is unwound, keeps the package-level import of ``ralph.workspace``
    # acyclic.
    from ralph.agents.registry import (  # noqa: PLC0415  # reason: lazy import breaks ralph.workspace<->ralph.mcp circular chain
        AgentRegistry,
    )
    from ralph.config.loader import (  # noqa: PLC0415  # reason: same lazy-import rationale
        load_config,
    )
    from ralph.mcp.artifacts.file_backend import (  # noqa: PLC0415  # reason: same lazy-import rationale
        DEFAULT_FILE_BACKEND,
    )
    from ralph.mcp.session_plan import (  # noqa: PLC0415  # reason: same lazy-import rationale
        resolve_effective_session_mcp_plan,
    )
    from ralph.policy.loader import (  # noqa: PLC0415  # reason: same lazy-import rationale
        load_policy_for_workspace_scope,
    )

    target_root = Path(target_root)
    if not DEFAULT_FILE_BACKEND.exists(target_root):
    ...
    if not DEFAULT_FILE_BACKEND.exists(target_scope.root):
        raise ValueError(
