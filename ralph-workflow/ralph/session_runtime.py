...
invocation wiring, resumable session environment, and optional system-prompt
materialization.
...
- The runtime is created via :class:`ManagedAgentSessionRuntime.open` (a
  ...
  bridge, optionally materializes a system prompt, and returns the runtime.
...
- All collaborators (workspace factory, MCP server starter, agent invoker,
  system prompt materializer, MCP bridge shutdown) are injectable through
...
- :class:`ManagedAgentSessionRequest` is a frozen dataclass that carries the
  ...
  optional pre-resolved :class:`SessionMcpPlan`, optional system-prompt
