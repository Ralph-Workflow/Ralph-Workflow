...
Call the `ralph_submit_artifact` tool with `artifact_type` set to `review_analysis_decision` and `content` set to either a native JSON object or a JSON-serialized string containing your decision payload.
