...
`ralph_validate_draft` runs the full `PlanArtifact` cross-section validator (depends_on cycle, intent_verb vs scope_item category, parallel_plan XOR work_units, shell-invocation guard, research/verify steps in AC.satisfied_by_steps, AC id pattern, non-empty `skills_mcp.skills`, 4 MB size cap) without writing `plan.json` and without deleting the in-progress draft. Returns `{"valid":true,"finalizable":true,"errors":[]}` on success or `{"valid":false,"finalizable":false,"errors":[{"message":"...","code":"...","repair":"..."}]}` on failure. If no draft exists yet, it returns `{"valid":false,"finalizable":false}` with `code="NO_PLAN_DRAFT"` instead of a false-green success. Cross-section invariants only run at `finalize_plan` in the write path; `ralph_validate_draft` exposes the same checks in a read-only path. Capability: `artifact.plan_read`.

### Net-new batched tool: `ralph_submit_plan_sections`

`ralph_submit_plan_sections` accepts one canonical agent-facing envelope: an object with an `entries` array, like `{"entries":[{"section":"summary","content":{"context":"Fix foo() out-of-range index handling after reading src/foo.py and tests/test_foo.py","intent":"Clamp foo() indexes and prove the regression with a focused test","intent_verb":"fix","scope_items":[{"text":"Add tests/test_foo.py::test_clamp_handles_out_of_range_index","category":"test"},{"text":"Update src/foo.py to clamp negative and oversized indexes without changing the public foo() signature","category":"bugfix"},{"text":"Run pytest tests/test_foo.py -q to prove the regression is fixed","category":"test"}]},"mode":"replace"}]}`. Do not submit a bare entries list in agent prompts or docs. The tool parses ALL entries BEFORE any merge; malformed JSON, unknown sections, and impossible modes reject the batch (`{"submitted":[],"failed_at":1,"error":"..."}`) and leave the draft unchanged. Schema-invalid but valid JSON is staged and returned in `validation_warnings`; this means the draft is accepted for repair, not finalizable. On success it returns `{"submitted":["summary"],"staged_sections":["summary"],"total_bytes":123,"validation_warnings":[],"staged":true,"section_valid":true,"can_repair":false}`. If warnings are present, `section_valid` is `false` and `can_repair` is `true`; fix the named section and run `ralph_validate_draft`. Use this when you have built complete, analysis-ready section payloads. The full cross-section validator still runs at `validate_draft` / `finalize_plan`. Capability: `artifact.plan_write`.

### Design sub-section

The `design` field is OPTIONAL. When present, it carries seven typed sub-models that bias the executor toward good Software Engineering practices. An explicitly empty design object (`{}`) is staged with a `validation_warnings` entry instead of being rejected, so agents can repair it later without losing the draft. Treat that warning as unfinished work: re-submit `section="design"` with real `acceptance_criteria`, `outcome`, or other design details before finalizing a non-trivial plan.
...
- `satisfied_by_steps cannot reference a research or verify step; step N is T for criterion ID` - fix: edit `design.acceptance_criteria.criteria[id=ID].satisfied_by_steps`; remove the research/verify step number and point only to concrete `file_change` or `action` steps that deliver the AC. Do not edit verify steps unless the plan semantics are actually wrong.
