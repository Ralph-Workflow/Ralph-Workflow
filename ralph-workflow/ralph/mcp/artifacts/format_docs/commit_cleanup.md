...
- **`.gitignore` and `.git/info/exclude` writes are atomic** (sibling-staging with a sha256(payload).hexdigest()[:16] content-derived hash plus os.getpid() suffix + `Path.replace()` per the pattern from `ralph/mcp/transport/agy.py:99-127`) so SIGKILL during cleanup leaves the files intact. The atomic helper in `ralph/git/operations.py::_atomic_append_text` reads and writes the staging file in bytes mode (`Path.read_bytes()` + `Path.write_bytes()`) so CRLF, BOM, and any other byte-level content is preserved through the round trip.
...
- **Atomic-append staging filenames are content-derived** (sha256(payload) hash plus os.getpid() suffix in `ralph/git/operations.py::_atomic_append_text`) so concurrent invocations cannot collide on the sibling staging file -- the previous `id(payload)`-derived suffix could collide whenever the caller constructed equal-id strings back-to-back (e.g. empty payload or shared string-literal dedup). The change is pinned by the `audit_agent_internal_paths` invariant on `ralph/git/operations.py` (present literals `hashlib.sha256(payload.encode(encoding)).hexdigest()[:16]` and `os.getpid()`; absent literal `id(payload)`).
- **Pre-emptive untrack at phase startup.** Before loading the artifact, the phase walks every file tracked in HEAD and runs `git rm --cached` on any path that matches the runtime-artifact allowlist (the same `is_agent_internal_path` predicate used by `_is_safe_to_delete`). This is a rock-solid safety net: even if the agent fails to identify engine-internal files, the artifact is empty, the artifact is missing, or the agent's `delete_file` actions get rejected, the files are still kept out of the commit. The untrack happens BEFORE the agent runs, so the agent's view of the diff no longer includes the untracked files. The working-tree files are left in place; the agent can follow up with a `delete_file` action if it wants them removed from disk. The placement of this call is pinned by `audit_agent_internal_paths._check_pre_emptive_untrack_placement` (between `ensure_git_initialized` and `_load_cleanup_artifact`, exactly once).
