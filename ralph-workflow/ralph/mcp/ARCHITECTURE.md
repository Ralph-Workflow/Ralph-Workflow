...
| `exec.py` | `handle_exec_command`, `run_command` — bounded subprocess execution routed through the reusable sandbox pool; `_build_effective_deps` wires `session.stream_tool_output` into `on_output_chunk` for SSE streaming |
