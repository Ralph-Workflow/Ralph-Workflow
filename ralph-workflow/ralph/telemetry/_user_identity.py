...
def _write_user_id_file(config_path: Path, user_id: str) -> None:
    ...
    content = (
        ...
        "#\n"
        "# To opt out: set the RALPH_DISABLE_TELEMETRY environment variable to a\n"
        "# truthy value (any of 1/true/yes/on, case-insensitive) before\n"
        "# invoking Ralph Workflow. This file is NOT an opt-out on its own:\n"
        "# if it is missing, Ralph Workflow will create a new random ID on the\n"
        "# next run only when telemetry is enabled.\n"
        "\n"
        f"[{_CONFIG_SECTION}]\n"
