...
conformance scope, and exclusions; uncertainty requires owner confirmation.
If the surface disappears, retain the file with a dated inactive decision and
reactivation trigger, or remove it through reviewed policy cleanup.
## Default requirements
* Semantic structure, names, roles, states, focus order, keyboard operation, contrast, zoom, motion preferences, and error identification MUST be addressed where applicable.
...
* Automated checks MUST complement, not replace, keyboard and assistive-technology review of material flows.
* New UI MUST meet the project's declared accessibility standard and MUST NOT introduce regressions.
## Project facts to resolve
The lines below are the verified project facts that agents rely on and keep current.
<!-- REPLACE-ME: record the standard, supported input modes, review process, and real gate, then delete this comment. -->
...
supports them; otherwise inspect platform accessibility APIs and document the limitation.
Agents MUST run declared commands and reviews, identify the tested platform and
flow, and report manual capability that was not available. Agents MUST NOT
claim assistive-technology evidence that was not performed.
## Verification
...
Review when the UI framework, component library, accessibility standard, or primary interaction pattern changes.
## Research basis
