...
_TYPE_IGNORE_ALLOWLIST: set[tuple[str, str]] = {
    ...
    # PEP 562 lazy module-load pattern: ralph.mcp.explore.graph /
    # ralph.mcp.explore.handlers use ``__getattr__`` / ``__dir__`` to
    # expose deferred sub-module symbols so the ``handlers.py`` and
    # ``graph.py`` hubs break circular imports without late imports.
    # mypy cannot narrow ``globals()`` or a dynamic attribute fetched
    # from a module-like proxy, so the dict/set comprehension and
    # runtime call site each carry the documented
    # ``# reason: autogenerated code`` marker with a ``misc`` ignore.
    # The ``_handlers_graph`` runtime dispatch through ``graph_module``
    # has the same problem on the call site and is allowlisted under
    # both ``operator`` (for the proxy ``.`` access) and ``misc``.
    ("graph", "misc"),
    ("handlers", "misc"),
...
def _collect_py_files(root: Path) -> Iterable[Path]:
    ...
    for path in root.rglob("*.py"):
        if any(
            part in _SKIP_DIRS or part.startswith(".venv") for part in path.relative_to(root).parts
        ):
            continue
        yield path
