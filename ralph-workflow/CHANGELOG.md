...
- **Streaming block grouping.** Consecutive `text` or `thinking` activity lines from the same worker are now grouped into `[content-start]`/`[content-continue]`/`[content-end]` (or `[thinking-start]`/`[thinking-continue]`/`[thinking-end]`) sequences. The `*-end` line carries a one-line headline summary of the accumulated block. `PlainLogRenderer.flush_blocks()` closes open blocks on phase transitions and pipeline stop.
- **`ralph.display.long_content_summary`** — opt-in headline extractor for oversized content. When `RALPH_LONG_CONTENT_SUMMARY=1` (or `true`/`yes`) is set and content exceeds 4000 display cells, a `↳ summary:` line with the first non-empty sentence (max 120 chars) is prepended before the condensed excerpt. No external AI call — the upstream provider already produced the text. See README "Long-content display" section.
