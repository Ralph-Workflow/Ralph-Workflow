...
- **refactor(mcp)!: plan validation errors fire only on the closed not-a-plan list plus the three named routing/transport classes** — the plan validator's error severity now applies only to documents recognizably not a plan (the `PLAN001` closed list: empty / markup-only / under 100 chars of actual content / a refusal, question-back, status message, tool output, stack trace, or placeholder) and to the three transport / routing classes the brief's "pipeline cannot proceed" clause covers (`SPEC002` missing `type` and `MD005`/`MD006`/`MD007` unparseable frontmatter for the spec registry; `SPEC001` / `PLAN020` size bound for the bounded MCP payload; `PLAN023` noop grammar for the analysis phase noop routing). Every content / shape finding that previously blocked (the markdown body grammar `MD002`, the `SPEC011`/`SPEC012` section-shape mismatches, the reference / graph findings `REF001`–`REF004`, the plan-shape findings `PLAN021` / `PLAN022` / `PLAN024`, the `PLAN020` non-routing error sites for type mismatch, schema_version, shell prefix and fan-out field strictness, and the pydantic branch of `SPEC010`) demotes to warning and follows the `what; the run cost is <cost>; resolve by <fix>` advisory convention. Other artifact types' severities are byte-for-byte unchanged because the policy is plan-scoped. The `ralph/phases/execution.py` plan gates (`_validate_plan_output` at line 204 and `_validate_plan_input` at line 252) now degrade plan content-shape failures instead of emitting blocking `artifact_validation_failure_event`s, and the pydantic / `specs/plan.py:1150` `{}`-collapse is dropped so a warnings-only document persists its raw mapped content. Breaking change for any out-of-tree consumer that relied on a demoted rule blocking submission; the in-repo skills, prompts, format doc, and tests are updated to the advisory model. Locked by `tests/mcp/test_md_plan_not_a_plan.py`, `tests/mcp/test_md_plan_advisory.py`, `tests/mcp/test_md_plan_chain_e2e.py`, `tests/mcp/test_md_plan_relaxation.py`, `tests/mcp/test_md_plan_spec.py`, `tests/test_plan_artifact_submit_plan_sections.py`, `tests/test_plan_artifact_validate_draft.py`, and `tests/pipeline/ -k plan`.
