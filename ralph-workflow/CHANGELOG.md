...
- **feat(display): redesign production console presentation** — activity rows now use fixed-RGB semantic carriers, previews own complete surfaces, and cell-width layout keeps live and redirected output readable across the supported display matrix ([`4077a8d3f`](https://github.com/Ralph-Workflow/Ralph-Workflow/commit/4077a8d3f)); locked by `tests/test_display_generated_scenes.py`, `tests/test_display_visual_floor.py`, and `tests/test_plain_renderer_level_styling.py`.
