...
- **fix(claude): tail subagent transcripts and route events to the watchdog** — `_subagent_transcript.py` discovers every `agent-*.jsonl` under the parent's `subagents/` directory and forwards parsed events through `record_subagent_work`, replacing the previous in-process-blank assumption that made the watchdog kill productive subagents every ~5 minutes. Locked by `tests/agents/invoke/test_subagent_transcript_tail.py` and `tests/agents/idle_watchdog/test_no_kill_under_real_subagent_work.py`.
...
- **fix(smoke): treat the parallel-subagent diagnostic as a non-fatal break** — the `not every subagent dispatch has a correlated result` and `no meaningful activity was observed after the subagent result` errors are surfaced in the report's "Observed breaks" cell but no longer gate `EXIT_CODE`; the harness still grades against the verdict plus the absence of fatal errors. With `RALPH_BROKER_SECRET` set, `ralph smoke-interactive-claude --subagents` and `ralph smoke-headless-claude --subagents` now report `Verdict: PASS` and `EXIT_CODE=0`. Locked by the existing `tests/test_cli_smoke.py` parity harness and the live reports under `.agent/tmp/`.
