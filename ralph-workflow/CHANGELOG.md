...
- **feat(agy): pass `--print-timeout 1h` on every headless invocation** — AGY's provider-owned print deadline (five minutes) terminated long, valid sessions around 298 s with `{"status":"ERROR","error":"timeout waiting for response"}` even while AGY was actively using MCP tools, and neither the idle watchdog nor MCP activity tracking can extend it. The flag is emitted declaratively via `AgyCommandBuilder.SPEC.extra_flags_before_prompt`, immediately before `--print`; one hour intentionally exceeds Ralph Workflow's own 55-minute hard session ceiling (`MAX_SESSION_SECONDS = 3300.0`), so Ralph Workflow remains the lifecycle authority. Locked by `tests/test_agy_command_builder.py` and `tests/agents/invoke/test_command_builder_spec.py`.
- **feat(agy): recover an incomplete exit with one bounded reprompt** — an AGY run that exits `rc=0` without the completion sentinel or a required artifact receipt (including runs that waited for input or asked for clarification in a non-interactive session) now raises the typed `AgyIncompleteExitError` and gets exactly one automatic recovery reprompt: a fresh invocation (AGY has no demonstrably resumable session) carrying the original task plus an explicit completion instruction (finish autonomously, submit via `ralph_submit_md_artifact`, call `declare_complete`). The bound is enforced in both `build_agent_recovery_plan` and the direct-MCP recovery loop, the reprompt is never treated as completion evidence, and a still-evidenceless run fails with an actionable error. Locked by `tests/recovery/test_agy_incomplete_exit_recovery.py` and `tests/test_agy_incomplete_exit_reprompt.py`.

### Fixed

- **fix(watchdog): stop Ralph Workflow's own bookkeeping from counting as agent activity — and from feeding the workspace monitor its own output** — the workspace monitor republishes `.agent/.workspace-awareness.json` on every counted change, and the change classifier rated that file (and `.agent/PLAN.md`, `PRODUCT_CRITERIA.md`, `CURRENT_PROMPT.md`, `active_runs.json`, `auto_integrate_in_progress.json`, `mcp.toml` ...) as `source` activity. Every publication was therefore itself a counted change: a live workspace showed the sidecar rewritten four times a second with a `paths` list containing its own name, and a rebase conflict-resolution run whose OpenCode resolver had exited with `declare_complete` kept reporting `last_activity_kind=workspace; last_activity_age_seconds≈0` on every status tick, so its 900 s inactivity ceiling could never fire. Every file directly under `.agent/` is now `cache` (scoped to the immediate parent, so `docs/PLAN.md` and `.agent/workers/<unit>/` are untouched), the engine-written `.agent/receipts`, `.agent/prompt_history` and `.agent/artifact-formats` trees join the cache list, and `WorkspaceMonitor.record_event` ignores the sidecar and its scratch file outright, whatever weight an operator gives `cache`. The same sweep found the rest of the class and fixed it in the same rule set: the runner's root-level `checkpoint.json` and the agy/cursor/kimi MCP overlays (`.agents/mcp_config.json`, `.cursor/mcp.json`, `.kimi-code/mcp.json`) are `cache` when the monitor supplies its workspace root, as are `.agent/logs` and the engine-written parts of a fan-out worker unit (`.agent/workers/<unit>/{tmp,logs,handoffs}`, its `worker-manifest.json`; the unit's `artifacts` tree is `artifact`, its checkout is untouched). Locked by `tests/agents/test_workspace_change_classifier.py` and `tests/agents/test_workspace_watch_scoping.py`.
- **fix(conflict-resolution): stop a finished resolver from being held, retried, or misreported after its kill** — four sibling defects on the same path. (1) Durable completion evidence was ignored for ordinary marker conflicts (`requires_completion_evidence` is only set for modify/delete decisions), so a resolver that had called `declare_complete` was waited on until stdout EOF and then until every liveness signal went quiet; the reader now honours the sentinel under `activity_only` supervision like any gated phase (`completion_evidence_gates_reader`). (2) The executor re-launched a resolver killed for inactivity up to `max_same_agent_retries` (10) times — invisible to the driver's chain/round budget — and the driver then spent the next candidate before re-reading the worktree; a conflict resolver now gets no executor-level retries (`recovery_attempts_for_effect`) and `_run_one_round` re-scans the checkout after a candidate that ran and ended badly, ending the round instead of re-prompting a conflict that no longer exists. (3) `AgentInactivityTimeoutError` / `SupervisionInfrastructureError` were swallowed into `AGENT_FAILURE`, so every inactivity kill was filed as `CANDIDATE_EXITED` with `last_activity_kind=none`; they now reach `invoke_resolution_agent` typed (`surfaces_supervision_error`). (4) `_classify_quiet` defaulted to `WAITING_ON_CHILD` on any strategy exception even after the process had exited; a dead parent now defaults to `ACTIVE`. Locked by `tests/agents/invoke/test_conflict_resolution_completion_liveness.py`, `tests/agents/invoke/test_conflict_resolution_activity_profile.py`, `tests/pipeline/test_conflict_resolution_executor_retries.py`, and `tests/test_conflict_resolution_worktree_rescan.py`.
- **fix(opencode): stop waiting on a dead OpenCode parent whose turn frames never balanced** — `OpenCodeExecutionStrategy` counted `step_start` against `step_finish` to keep a turn with a buffered native `task` alive, but OpenCode 1.18 re-emits `step_start` for a retried message (its own store shows two `step-start` parts and one `step-finish` for the same message) and its final turn ends with the process exit, its closing `step_finish` reaching the store but not stdout. The count therefore never returned to zero, `classify_quiet` answered `WAITING_ON_CHILD` forever, and the conflict resolver's deadline-free drain kept waiting after `process EXITED rc=0` until the inactivity ceiling. Open steps are now keyed by `part.messageID` (frames without one fall back to a counter) and the reader tells the strategy when stdout reached EOF (`observe_stream_end`), releasing every open step because no buffered frame can still arrive. A completed `task` frame also now acks the child under `state.metadata.sessionId` — the id the session-store probe registers it by — instead of only its `callID`, so a finished native subagent no longer counts as `running` until its 45 s progress TTL prunes it. Locked by `tests/test_opencode_session_execution_quiet_parent_with_live_child.py`, `tests/agents/invoke/test_process_reader_opencode_stream_end.py`, and `tests/agents/execution_state/test_opencode_native_task_session_ack.py`.
- **fix(opencode): stop killing a parent whose native `task` subagent legitimately runs for many minutes** — `opencode run --format json` (1.18.x) emits `step_start` and then nothing until the `task` tool completes, so a silent parent carried no first-party evidence and the idle watchdog fired `NO_PROGRESS_QUIET` at 240 s (no OS descendants) or `CHILDREN_PERSIST_TOO_LONG` at 600 s (child shelling out), while real subagents run 10-15 minutes. `ralph.agents.invoke.opencode_subagent_sessions` now reads OpenCode's own session store (`opencode.db`, `session.parent_id` -> child, `part.time_updated`) read-only and forwards each new child part as demonstrated work: the watchdog's `subagent` channel plus an idle-baseline reset, and `OpenCodeExecutionStrategy.record_native_child_progress()` for the child-liveness registry. A child that stops writing still reaches the standard ceilings, and a single tool call inside the child that writes nothing for the idle window is as silent to the probe as to the stream; the session ceiling is unchanged. Locked by `tests/agents/test_idle_watchdog_opencode_native_task.py`, `tests/agents/invoke/test_opencode_subagent_sessions.py`, `tests/agents/invoke/test_process_reader_opencode_subagent_probe.py`, and `tests/agents/execution_state/test_opencode_native_child_progress.py`.
- **fix(mcp): give Ralph Workflow the OpenCode MCP servers its operator actually runs** — OpenCode was the one supported transport with no `load_existing_<agent>_upstream_servers`: `OpencodeRuntimeResolver` seeded `build_opencode_provider_config` from the `OPENCODE_CONFIG_CONTENT` environment variable alone, which is normally unset, so the computed upstream set was empty and Ralph Workflow never learned that the operator had any MCP servers at all. Nothing was broken for the operator — `Config.loadInstanceState` in the OpenCode 1.18.25 binary DEEP-MERGES its config sources, so their servers keep reaching the agent — but Ralph Workflow could not name them, in either mode. New `load_existing_opencode_upstream_servers()` reads every location OpenCode itself loads, verified against the binary and `opencode debug config`: the global config dir (`config.json`, `opencode.json`, `opencode.jsonc`), `$OPENCODE_CONFIG`, project `opencode.json`/`.jsonc` walked to the worktree root, `<dir>/.opencode/`, `~/.opencode`, `$OPENCODE_CONFIG_DIR`, and the managed config dir — honouring `$OPENCODE_DISABLE_PROJECT_CONFIG`, OpenCode's merge precedence, `enabled: false` / `disabled: true`, JSONC comments and trailing commas, and dropping the reserved `ralph` name before normalization; an unreadable or malformed file warns with its path instead of returning an empty map indistinguishable from "operator has no servers". OpenCode also spells a stdio server's command as a LIST (`["bunx", "@angular/cli", "mcp"]`), which the string-only normalizer dropped outright, so every `type: "local"` server was invisible even when it WAS in the payload. Those servers are deliberately NOT re-exposed as `ralph_upstream__*` proxies: the proxy contract exists to give back what a strict provider config takes away, and OpenCode's deep merge takes nothing away, so a proxy would show the agent a second copy of every tool, spawn a duplicate stdio child per local server, and gate nothing (the native name still works). Ralph Workflow now says so instead, once per run, naming the servers that run outside its capability gate. Locked by `tests/mcp/test_opencode_upstream_discovery.py`.
- **fix(claude)!: stop passing `--strict-mcp-config`, which deleted the operator's own MCP servers** — per `claude --help` the flag means "Only use MCP servers from --mcp-config, ignoring all other MCP configurations", so every MCP server the operator installed in their harness vanished for the run. Ralph's proxy bargain (discover them, re-expose as `ralph_upstream__*`) cannot cover a server with no on-disk definition: `claude mcp list` on the maintainer's machine reports four claude.ai account connectors (Notion, Gmail, Google Drive, Google Calendar) that are delivered by the signed-in account and exist in no file, so Ralph could neither discover nor restore them. Ralph now passes `--mcp-config` alone, which ADDS its server on top of Claude's own sources; the accepted trade is that operator servers reach Claude natively, outside Ralph's capability gate. Ralph's own `--tools` / `--allowedTools` restriction is unchanged. Locked by `tests/agents/invoke/test_claude_strict_mcp_config_gap.py` and `tests/agents/invoke/test_claude_tool_restriction_regression.py`.
- **fix(claude): fail closed when the native-tool restriction cannot be computed** — `--tools`/`--allowedTools` were emitted only for a non-empty discovered MCP tool list, while `--mcp-config` + `--strict-mcp-config` went out unconditionally, and a failed `tools/list` (slow MCP start, dropped connection) was swallowed into an empty tuple with a `warning`. A transient discovery failure therefore launched Claude with the operator's own MCP servers stripped AND every native tool enabled — the opposite of the intended posture, and contradicting the prompt Ralph sends. Empty-because-discovery-failed is no longer indistinguishable from empty-because-no-restriction-applies: the discovery error now aborts the launch with an operator-visible `AgentInvocationError` naming the endpoint and the cause. Locked by `tests/agents/invoke/test_claude_tool_restriction_regression.py`.
- **fix(cli): stop the two Claude smokes from pinning a model the pipeline never runs** — `smoke-interactive-claude` and `smoke-headless-claude` exposed no `--agent` at all and were hardcoded to `claude/haiku` and `claude-headless/haiku`. Taking no `--agent` made them exempt by construction from the guard that forbids a pinned `<transport>/<model>` default, so they were the last two smokes testing a model of Ralph's choosing rather than the operator's — and would have failed for reasons unrelated to Ralph the moment that model id was retired, exactly as the Codex smoke did with `gpt-5-flash`. Both now accept `--agent` and default to the operator's own `[agent_chains]` entry. `resolve_smoke_harness_spec` matched interactive Claude by exact string against that one alias, so any other Claude model raised `ValueError` instead of running; it now accepts the whole `claude` / `claude-headless` family the way the agy, cursor and opencode branches already did, while `claude/haiku` keeps its legacy on-disk layout. Locked by `tests/test_cli_harness_parity.py` and `tests/pipeline/test_claude_harness_spec_resolution.py`.

- **fix(mcp): stop the Codex home mirror from copying gigabytes per invocation** — Ralph symlinks every entry of `~/.codex` into the throwaway `CODEX_HOME` and fell back to a full copy on `OSError`. Symlinking needs developer mode or admin on Windows, so there the fallback is the only path — and a real `~/.codex` is mostly session databases (`logs_*.sqlite`, `state_*.sqlite`, `thread_history_*.sqlite`) running to several gigabytes, copied afresh on every codex call. That reads as a hang and can fill the disk. An entry that cannot be symlinked is now copied only when it fits a budget sized well above settings, auth and plugins and well below session history; anything larger is skipped with a warning naming it, since Codex recreates its own session state. Locked by `tests/mcp/test_codex_config_synthesis.py`.

- **fix(mcp): make an operator's MCP config recoverable after a killed run** — the cursor, kimi and agy transports overwrite a user-global file they do not own (`~/.cursor/mcp.json`, `$KIMI_CODE_HOME/mcp.json`, `~/.gemini/**/mcp_config.json`) and restored it in a bare `finally`. A SIGKILL, an OOM kill, a reboot or a power cut skips `finally`, so the operator was left with their own MCP servers erased and a single Ralph entry pointing at a dead localhost port — permanently, because the next run then snapshotted that corpse as "the original" and restored it again. Each overwrite is now preceded by a durable `<name>.ralph-backup` record (operator bytes plus the digest of the overlay), and every overlay begins by reclaiming a record an earlier run never applied, so a killed run self-heals on the next invocation. The reclaim is content-gated, not age-gated: it restores ONLY while the file on disk is still byte-identical to the overlay the record names, so an operator's own repair, an IDE rewrite or a deletion supersedes the record instead of being clobbered by it. New shared module `ralph/mcp/transport/config_overlay.py`. Locked by `tests/mcp/test_mcp_config_overlay_recovery.py`.

- **fix(mcp): serialize the cursor and kimi config overlays across processes, not just threads** — both guarded their snapshot/write/restore transaction on a shared user-global file with a `threading.Lock`, which is invisible to a second Ralph process: two runs would each snapshot, each overwrite, and whichever restored last would write back the wrong content. They now take the same bounded advisory `flock` sidecar agy already had, extracted into `ralph/mcp/transport/config_overlay.py` and shared by all three; a contended overlay fails closed with a lock-timeout error rather than racing the holder. Locked by `tests/mcp/test_mcp_config_overlay_recovery.py` and `tests/test_agy_config_overlay_cross_process_e2e.py`.

- **fix(mcp): stop `--strict-mcp-config` from deleting MCP servers Ralph cannot give back, silently** — the documented Claude bargain is that everything Ralph strips comes back as a `ralph_upstream__*` proxy. That is impossible for a server with no on-disk definition: a claude.ai account connector is delivered by the signed-in account and appears in no file under `~/.claude`, `~/.claude.json`, or `~/.config`, and the same goes for a session-only `--plugin-dir`/`--plugin-url` plugin. On a machine where `claude mcp list` reported 8 servers, Ralph discovered 4 and removed the other 4 (Notion, Gmail, Google Drive, Google Calendar) without a word. Ralph now consults `claude mcp list` once per run, diffs it against what it discovered, and logs a WARNING naming every server the run is taking away. The probe is bounded (20s), routed through the managed process executor, memoized so it cannot run per agent cycle, and fully injectable via `claude_cli_mcp_server_lister`; a missing, slow, or failing CLI reports nothing rather than failing the run. `--strict-mcp-config` itself is unchanged — the security posture is the repo owner's call. Locked by `tests/mcp/test_claude_transport.py` and `tests/agents/invoke/test_claude_strict_mcp_config_gap.py`.

- **fix(mcp): make Claude unsafe mode see the servers strict mode learned to find** — the unsafe-mode merge in `ralph/mcp/transport/common.py` re-implemented Claude discovery from the two plain `mcpServers` files instead of calling `load_existing_claude_upstream_servers`, so the mode whose entire purpose is preserving the operator's servers stayed blind to plugin-provided and local-scope (`claude mcp add`) servers. It now calls the single public loader, and the duplicated `pop`/`normalize` is gone. Locked by `tests/mcp/test_claude_transport.py`.

- **fix(mcp): stop re-exposing the Claude MCP servers an operator turned off** — Claude does not trust a checked-in `.mcp.json` on sight; it asks, and records the refusal per project in `~/.claude.json` as `projects.<workspace>.disabledMcpjsonServers`. Ralph ignored that list and proxied the declined servers anyway, handing the agent a capability the operator withheld. The list is now honoured against the `.mcp.json` contribution alone, so a declined project server is dropped while a same-named user-scope or local-scope server — a different server that Claude still runs — survives. Locked by `tests/mcp/test_claude_transport.py`.

- **fix(mcp): stop deleting the MCP servers an operator installed through a Claude plugin or `claude mcp add`** — Ralph runs Claude with `--strict-mcp-config`, so the config it generates is the only MCP source Claude sees and everything Ralph fails to discover is gone rather than re-exposed as a `ralph_upstream__*` proxy. Discovery read three files (`~/.claude.json`, `<workspace>/.mcp.json`, `<workspace>/.claude.json`) and missed two sources Claude itself loads: every enabled plugin's `<plugin-root>/.mcp.json`, and `~/.claude.json` → `projects.<workspace>.mcpServers` — which is where a plain `claude mcp add` writes, `--scope` defaulting to `local`. This is the OpenCode `--pure` defect again: Ralph may add its own MCP server, it may not strip the operator's. Plugin servers keep Claude's own namespacing as `plugin_<plugin>_<server>` so one cannot silently replace a same-named user server, and a config file that cannot be parsed now warns with its path instead of returning an empty map indistinguishable from "no servers". Locked by `tests/mcp/test_claude_transport.py`.

- **fix(opencode): stop stripping the `opencode/` model prefix three times** — the Ralph alias prefix was removed at alias resolution (correct) and then twice more downstream, in the command builder and in the invoke preflight. opencode publishes a provider literally named `opencode` (`opencode models` lists `opencode/big-pickle`, `opencode/nemotron-3-ultra-free`, …) and parses `-m` as `provider/model`, so the alias `opencode/opencode/big-pickle` reached the CLI as `-m big-pickle` — read as `providerID='big-pickle'` with an empty model id, making every model under that provider unselectable. The over-stripping also removed the `/` before the local-model preflight could see it, so the run died at the CLI instead of failing fast. The prefix is now stripped exactly once, in `ralph/agents/registry.py`, and the shared extractor `opencode_model_id_from_flag` in `ralph/api/opencode.py` returns the id verbatim. Locked by `tests/agents/invoke/test_opencode_model_flag_regression.py`.
- **fix(opencode): shell-quote the alias model flag so it cannot smuggle argv tokens** — the dynamic `opencode/<provider>/<model>` alias interpolated the model id into `-m {}` with no `shlex.quote` (unlike the kimi, claude, codex, and cursor aliases) and the command builder re-tokenized it with `str.split()`, returning every part when the count was not two. `opencode/minimax/MiniMax-M3 --agent plan` therefore emitted `['-m', 'minimax/MiniMax-M3', '--agent', 'plan']`. The registry now quotes the value and the builder tokenizes with `shlex.split`, rejecting any shape that is not one flag plus one non-flag value — the fail-closed posture `_tokenize_pi_model_flag` already documents for Pi. Locked by `tests/agents/invoke/test_opencode_model_flag_regression.py`.
- **fix(opencode): drop the nonexistent `--json-stream` output flag and declare the override discard** — opencode 1.18.25 has no output-format flag beyond the `--format json` Ralph already passes, yet both the builtin spec and the command-builder spec carried `output_flag="--json-stream"`. It was inert only because the builder guarded on `"opencode" not in base_argv[0]` — a name sniff that also silently threw away any operator `[agents.opencode] output_flag`. The flag is gone and the drop is now declared by `CommandBuilderSpec.honors_output_flag=False`. Locked by `tests/test_agents_invoke.py`.
- **fix(opencode): stop disabling `codesearch`, which is not an opencode tool** — `OPENCODE_NATIVE_TOOLS_TO_DISABLE` listed a permission key that appears nowhere in the 1.18.25 binary, so the override named a tool that never existed. Locked by `tests/test_agents_invoke.py`.
- **fix(smoke): default the harness to the agent the operator's own pipeline runs** — every `smoke-interactive-*` command pinned a hardcoded `<transport>/<provider>/<model>` alias, spelled twice: once in the Typer option in `ralph/cli/main.py` (the value the CLI actually used) and again in the command function's signature in `ralph/cli/commands/smoke.py` (shadowed, so the two could silently disagree). The live pipeline is driven by the operator's `[agent_chains]`, so a harness PASS was evidence about a model the pipeline never ran — and the pinned id went stale the moment a provider retired it, which is exactly how the Codex harness ended up failing on `gpt-5-flash` HTTP 400 for a reason unrelated to Ralph. With no `--agent`, the default now resolves to the first alias in the operator's chains (chains in config declaration order, agents in fallback order) that maps to that transport, falling back to the bare transport alias, which passes no `--model` and so cannot go stale. An explicit `--agent` still wins. Locked by `tests/test_cli_harness_parity.py` and `tests/test_opencode_defaults.py`.

- **fix(smoke): make the `RALPH_*_BINARY` override reach the process for every transport** — four transports carried a hand-copied trio of override helpers and they were wired up inconsistently: the shared harness applied only AGY and Cursor, while the Kimi and OpenCode commands applied theirs to a config the harness then threw away (it takes only `agent_name` and re-resolves everything itself), so `RALPH_OPENCODE_BINARY` logged "Using ... override" and had no effect at all. Cursor's fared no better for a dynamic `cursor/<model>` alias, which resolves from the built-ins rather than from `config.agents`. The four copies collapse into one table in `ralph/cli/commands/smoke_binary_override.py`, applied in exactly one place inside the shared harness and injected under the resolved alias so it reaches the spawned process identically for every transport; the dead per-command application is gone. Locked by `tests/test_cli_harness_parity.py`.

- **fix(smoke): stop telling every transport how to use AGY's tools** — the prompt bullet instructing the agent to create the workspace file through `call_mcp_tool` rather than AGY's own `write_to_file` was emitted unconditionally, so a Claude, Codex or OpenCode run was told about a dispatcher and a native tool neither of which it has. The bullet is now gated on the AGY transport, like the submission and fallback bullets beside it, and bare `agy` resolves to the base AGY harness layout the way bare `cursor` already did. Locked by `tests/test_cli_smoke.py`.

- **fix(mcp): refuse a Codex run rather than silently swapping the operator's provider** — Ralph redirects `CODEX_HOME`, so the config it synthesizes is the only one Codex sees. When the operator's `~/.codex/config.toml` could not be parsed, or was nested too deeply for `tomli_w` to re-serialize, Ralph logged an error and carried on from an empty base — discarding `model_provider`, every `[model_providers.*]` block and every `[agents.*]` block, then running against a different provider on different credentials and reporting success. Both paths now raise `CodexConfigError` and name the file. Silently substituting settings the operator never chose is the same harm as disabling their plugins, only quieter. Locked by `tests/mcp/test_codex_config_synthesis.py`.

- **fix(invoke): stop disabling the operator's OpenCode plugins** — Ralph forced `--pure` on every OpenCode invocation, which runs OpenCode without external plugins. A plugin is often what supplies the operator's model *provider*, so the model they had configured became unresolvable and every run died with `UnknownError: Unexpected server error` before doing any work. Measured against the installed CLI: `opencode models` lists 738 entries, `opencode --pure models` lists 398 and none from the plugin-provided provider. Ralph runs an operator's coding harness with the extensions they installed; it may add its own MCP server, but it does not strip theirs. The live OpenCode smoke goes from `DEGRADED`/exit 1 to `PASS`/exit 0 on this change alone.

- **fix(process): give a spawned child a `PWD` that names its own working directory** — `cwd=` moves the child but leaves the inherited `PWD` pointing at wherever Ralph was launched from, and tools that trust `PWD` over `getcwd()` then work on the wrong tree. OpenCode resolves its project root as `resolve(process.env.PWD ?? process.cwd())`, so running Ralph from outside the workspace — a linked worktree, `--workspace` elsewhere, a wrapper script — silently pointed it at a different repository while Ralph reported on the intended one. `PWD` is now realigned (and the stale `OLDPWD` dropped) at the single spawn seam every agent goes through, which is what `ralph/mcp/tools/exec.py` had always done for the exec tool. Locked by `tests/process/test_spawn_env_pwd_alignment.py`.

- **fix(mcp): stop a leftover `ralph` entry from aborting every Claude, OpenCode and Nanocoder run** — `ralph` is a reserved upstream name that `normalize_upstream_mcp_servers` raises on, and three more transports handed the operator's own MCP server map to it unfiltered: Claude on every invocation once an endpoint is set (`~/.claude.json`, `<workspace>/.mcp.json`, `<workspace>/.claude.json`), OpenCode in its default restricted mode, and Nanocoder in both modes — so an operator who had ever pointed one of them at Ralph by hand, or copied back a config Ralph synthesized, could not run that agent at all. Ralph always replaces its own entry with the live endpoint, so the stale one is now dropped before normalization the way Codex already dropped it, and the shared loader's unreachable opencode branch — dispatched away long before it — is gone. Locked by `tests/mcp/test_claude_transport.py`, `tests/mcp/test_opencode_transport.py`, `tests/mcp/test_nanocoder_transport.py` and `tests/test_agents_invoke.py`.

- **fix(mcp): stop the Codex upstream probe from writing TOML it cannot read back** — `--check-mcp` appended the probed server as a hand-written `[mcp_servers.<name>]` section, so text with no escaping decided the result: a `.` in a server name silently nested it under a different table, a `"` or `\` in a URL closed the string early, and an operator's inline `mcp_servers = {...}` collided with the appended header to reproduce the very "duplicate key" abort the probe exists to rule out — each surfacing as a bare `TOMLDecodeError` instead of a probe error. The entry is now merged as a mapping and re-serialized, the same way the Codex transport stopped splicing. Locked by `tests/mcp/test_agent_transport_probe.py`.

- **fix(install): stop a reinstall from deleting the interpreter out from under a live run** — `rdev` runs *from* the dev snapshot, so `<snapshot>/.venv/bin/python` is the running `sys.executable` and the binary Ralph re-spawns for the MCP server subprocess; the installer replaced the snapshot by removing the whole directory, which took that binary with it and killed any run in flight with `FileNotFoundError: .../.venv/bin/python` followed by an unrecoverable restart-budget exhaustion. The virtualenv is never copied from the source checkout either, so deleting it also forced a from-scratch rebuild for nothing. A reinstall now clears the snapshot entry by entry and leaves the virtualenv standing for `uv sync` to reconcile in place. Locked by `tests/test_install.py`.

- **fix(mcp): synthesize a Codex config that cannot carry a duplicate key** — Ralph spliced its required settings into a copy of the operator's `~/.codex/config.toml` as text, so an operator who already set one of the keys Ralph owns (`multi_agent` in `[features]`, say) got that key stated twice and Codex aborted with `Error loading config.toml: ... duplicate key` before emitting a single event. The config is now parsed, merged as a mapping and re-serialized: a mapping cannot hold a key twice, so the failure is structurally impossible rather than merely avoided, and the result is round-trip-checked on every invocation. Locked by `tests/mcp/test_codex_config_synthesis.py`.

- **fix(mcp): stop a leftover `[mcp_servers.ralph]` from aborting every Codex run** — `ralph` is a reserved upstream name that `normalize_upstream_mcp_servers` raises on, and Codex handed the operator's `mcp_servers` to it unfiltered, so an operator who had ever pointed Codex at Ralph by hand — or copied a synthesized config back to `~/.codex` — could not run Codex at all. Ralph replaces its own entry with the live endpoint, so the stale one is now dropped before normalization the way every other agent already dropped it. Locked by `tests/mcp/test_codex_config_synthesis.py`.

- **fix(agents): recognize the session id Codex actually announces** — Codex calls a session a "thread" and names it once, on the first frame of `codex exec --json`, as `{"type": "thread.started", "thread_id": ...}`. Neither the frame type nor the `thread_id` spelling was known to the extractor, so every Codex run was graded "session ID was not observed" and could not be resumed even though its very first line carried the id. `thread_id` is read only for session-announcing frames, so an item or turn frame carrying one cannot pose as the announcement. Locked by `tests/agents/invoke/test_codex_session_id_extraction.py`.

- **fix(cli): stop pinning the Codex smoke to a model that no longer exists** — the default `codex/gpt-5-flash` failed the whole smoke with an HTTP 400 from the provider rather than a Ralph fault, and any hardcoded model id goes stale the same way as providers retire them. The smoke now defaults to the bare `codex` alias, which passes no `--model` and follows whatever model the operator has configured.

- **fix(conflict-resolution): resolve a paused rebase instead of dropping the resolver** — the integration seam built the MERGE resolver and discarded it whenever no merge was in progress, so a paused REBASE (the commoner blocking state) reached a resolver that was never invoked and the run exited with zero attempts; a paused rebase now goes to the rebase-stop resolver, an unreadable rebase state falls back to git's own on-disk markers rather than answering "no rebase", and the wrong resolver is no longer built to be fallen back on. Locked by `tests/pipeline/test_run_loop_resume_prefers_available_agent.py`.

- **fix(conflict-resolution): judge a round by the worktree, not the exit status** — every resolver is told "Ralph Workflow re-scans every listed path textually after you return; your own report is never the evidence", and the round then demanded that the invocation succeed as well, so an agent that resolved every marker and ended its session badly had finished work discarded and the conflict handed to the next candidate until the rounds ran out. The markers decide, once an agent has actually run. Locked by `tests/test_conflict_resolution_pipeline.py`.

- **fix(conflict-resolution): give a modify/delete conflict to the resolver, with proof** — it was escalated on sight because such a conflict carries no markers, which also means the usual textual proof is satisfied before anyone touches it; it is now classified as a DECISION that requires declared completion, so an agent that did nothing cannot silently land "keep" and reverse the other side's deletion. Locked by `tests/test_conflict_resolution_outcome_judging.py`.

- **fix(recovery): classify conflict-resolution outcomes instead of flagging them unrecognised** — the resolver types every outcome itself but has no exception to hand over, so its reasons arrived as bare prose and were logged as "Ambiguous failure classification ... [flagged_for_review=true]": an unrecognised fault, for the one failure Ralph understands best. A failed attempt is an agent fault, an unreachable provider is environmental, an agent the registry cannot produce is configuration. Locked by `tests/test_conflict_failure_classification.py`.

- **fix(auto-integrate): stop Ralph's own infrastructure faults from ending the run as a spent resolver** — a dead tool surface, a transport loop or an unanswered supervision relay was recorded as `resolution_exhausted`, durable terminal evidence that the RESOLVER had given up, so the next seam refused to try for a conflict no agent had actually failed at. `charge_conflict_budget` — written in three places and read nowhere — is now what decides it, and it is reset per stop so it describes the stop that failed. Locked by `tests/test_conflict_resolution_futile_loop.py`.

- **fix(conflict-resolution): stop reporting a refusal no agent can make** — a resolution session has no tool with which to decline a conflict (`declare_complete` is its only completion signal), so `CANDIDATE_DECLINED` named something that cannot happen while hiding what did. It is replaced by the states that are real: `ATTEMPT_FAILED` (the invocation ran and failed), `RESOLUTION_INCOMPLETE` (it succeeded and left markers), `CANDIDATE_EXITED` (it never got going) and `CANDIDATE_UNAVAILABLE` (the registry cannot produce it) — each answered by trying again rather than accepting a verdict. Locked by `tests/test_conflict_resolution_phase_parity.py`.

- **fix(conflict-resolution): stop reporting a candidate that never ran as one that answered** — the executor returns the same non-success event for failures that happen BEFORE any agent runs (a name the registry cannot produce, a dispatch the policy refuses), which is why "invoking X" was followed instantly by X apparently reading the conflict and refusing it. A candidate the registry cannot produce is now refused up front as `CANDIDATE_UNAVAILABLE` and handed over to the next one, and a non-success with no observed agent activity is an exit rather than an answer. Locked by `tests/test_conflict_resolution_phase_parity.py`.

- **fix(run-loop): stop letting one exhausted conflict end every later run** — `resolution_exhausted` was an unconditional stop, so the loop exited without invoking a resolver at all, refusing conflicts the record was never about. The stop now consults the same conflict budget the post-commit seam uses: a conflict that has CHANGED, or one whose anti-thrash budget still allows an attempt, reaches the resolver; only a still-binding exhaustion ends the run. Locked by `tests/pipeline/test_run_loop_resume_prefers_available_agent.py`.

- **fix(conflict-resolution): stop ending a round with no candidate invoked** — a round walked FORWARD from `chain_cursor` and stopped at the end of the chain, so a round beginning past the last candidate (or with dead surfaces ahead of a live one) invoked nobody and was still reported as a resolver's decline; the walk is now circular, each live candidate gets one turn per round, and `begin_resolution_stop` clears the cursor, terminal reason and attempt evidence a session would otherwise carry from the previous rebase stop into the next. `terminal_reason` is cleared before each candidate so an infrastructure fault marks the agent that produced it rather than the next one to fail. Locked by `tests/test_conflict_resolution_phase_parity.py`.

- **fix(conflict-resolution): tell a failed attempt apart from an agent that never ran** — a non-success agent event was reported as a decline whatever caused it, so pi exiting on an exhausted context or an unreachable provider read as a resolver refusing the conflict; the captured retry intent is now consumed for the real reason, reported as the new `CANDIDATE_EXITED`, and its `skip_same_agent_retries` advances the chain instead of retrying a futile candidate. Each failed attempt is also classified through `RecoveryController` exactly once — the driver, which knows the chain — where classifying in both places had spent a transient failure's whole same-agent retry budget on one attempt. Locked by `tests/test_conflict_resolution_phase_parity.py`.

- **fix(conflict-resolution): stop killing a resolver for quoting this repository** — `classify_ralph_origin_fault` matched the prose "tool surface", "tool service" and "repeated identical" against activity events that carry the agent's OWN output, so a resolver reading Ralph's source was recorded as a dead tool surface for the rest of the rebase — fatal with the shipped one-agent chain. Only tokens Ralph itself emits (`transport_loop_detected`, `SUPERVISION_INFRASTRUCTURE_FAILURE`) now classify. Locked by `tests/test_conflict_resolution_futile_loop.py`.

- **fix(pipeline): let pi fall over when its provider is unreachable** — `PiProviderFailureExitError` set `skip_same_agent_retries` on itself but was missing from `resolve_retry_intent`'s allowlist, so the intent the reducer's fallover reads was never built and pi was relaunched against the same dead provider. Locked by `tests/test_resolve_retry_intent.py`.

- **fix(run-loop): budget the conflict that is actually on disk** — the startup integration-resolution seam consulted the resolver budget with no `ConflictIdentity`, spending the default that matches every conflict, so a genuinely new conflict inherited an older one's exhausted count and was escalated without a resolver running — while the operator line called it "this unchanged conflict". Locked by `tests/pipeline/test_run_loop_resume_prefers_available_agent.py`.

- **fix(process): keep execve-illegal NUL bytes out of every spawn** — `argv[1:]` is stripped of embedded NULs (an agent prompt carrying a git diff of a source file with a literal NUL used to abort the phase with `ValueError: embedded null byte`), while `argv[0]`, env and cwd are rejected by name with a truthful `FAILED` record. Because the strip runs after validation, every argv validator now rejects a NUL-hidden token: `git_diff` args, `exec` params, and the `unsafe_exec` VCS denylist — whose script scanner also drops NULs before scanning, since `sh` does. `git_show` additionally rejects a flag-shaped ref (`--output=` let a read-only tool write files outside the workspace). A NUL is dropped from commit-message prose before git writes a `nulInCommit` object, rejected in commit path fields rather than silently naming a different file, and named truthfully in a staged path, a git `cwd` param, and a `read_file` path instead of surfacing as "outside repository root", a bare `lstat` error, or a retryable `-32603`; a commit artifact that fails the grammar is reported as invalid rather than empty. Locked by `tests/process/test_spawn_nul_sanitization.py`, `tests/test_spawn_nul_real_execve_e2e.py`, `tests/test_tool_git_read_parse_git_diff_params.py`, `tests/test_tool_git_read_parse_git_show_params.py`, `tests/test_tool_git_read_nested_repo.py`, `tests/test_tool_exec_parse_exec_params.py`, `tests/test_tool_unsafe_exec_handle.py`, `tests/test_tool_workspace_handle_read_file.py`, `tests/test_commit_message.py`, and `tests/test_pipeline_runner_execute_commit_effect.py`.
...
- **feat(smoke): grade MOCK_AGY_BEHAVIOR negative selectors through the full lifecycle smoke** — the v1.1.13 mock now supports deterministic `no_output`, `malformed_stream`, `failed_result`, `missing_dispatch`, `missing_result`, `missing_artifact`, and `missing_completion` selectors, each pruning exactly one contract signal, and the parameterized subprocess smoke asserts a non-zero CLI exit plus its diagnostic. Locked by `tests/test_smoke_agy_full_lifecycle_e2e.py` and `tests/_support/mock_agy.py` / `tests/_support/mock_agy_v1_1_13.py`.
...
- **fix(smoke): recognize Claude system/init frame and latch evidence ceiling** — `_advertised_tool_names_from_init_frame` parses Claude Code's `type: system`, `subtype: init` frame and latches `Provenance.WIRE` across transcript FIFO eviction. Locked by `tests/test_evidence_provenance_lattice.py`.
- **fix(mcp): serve every run from the worktree's own MCP server build** — the lifecycle's `_subprocess_env` prepended the parent package root to the child `PYTHONPATH`, but an outer launcher that exports a stale `ralph` install (e.g. a pipx site-packages) still sorted ahead in `python -m` resolution, so the spawned server ran pre-schema-flavor code and Moonshot rejected the composed root tool schemas with HTTP 400 on every live Kimi smoke. Every inherited `PYTHONPATH` entry that provides a `ralph` package is now replaced with the parent's own package root. Locked by `tests/test_mcp_lifecycle_pythonpath_isolation.py`.
- **fix(mcp): bind `/health` attributes at `_FallbackHttpServer` construction** — `health_probe_fn` / `metrics` were class-level annotations only, so the production standalone runtime's real `GET /health` route raised `AttributeError` when the smoke supervisor probed it; both are now constructor-owned defaults. Locked by `tests/mcp/test_fallback_standalone_health_route.py`.
- **fix(display): stop grading Ralph's own PTY harness input echoes as raw-transcript corruption** — `[claude turn boundary]` (injected into the PTY line reader's queue at the interactive exit boundary) and `/exit` (typed into the agent PTY and echoed by the tty line discipline) land verbatim in every interactive transport's raw capture; the `NON_JSONL` corruption detector graded both as breaks, so each live AGY interactive smoke exited 1 with "raw transcript corrupted" while the wire frames were intact. `is_harness_input_echo` now skips exactly those two lines in `detect_raw_log_breaks` (exact match only — a line merely embedding a marker still breaks), and `TURN_BOUNDARY_MARKER` is centralized in `ralph/display/raw_overflow.py` for the nanocoder parser and `_session` imports. Locked by `tests/test_raw_transcript_corruption_detection.py`.
- **fix(prompts): align the commit-message Jinja templates with the tightened commit-prompt contract** — `commit_message.jinja` and `commit_simplified.jinja` now open with "Produce and submit one valid `commit_message` artifact for the pending diff." (opencode: "Produce one valid `commit_message` artifact for the pending diff.") and state the restriction as "The only state-changing tools allowed are", matching the black-box contract. Locked by `tests/test_prompts_commit.py`.
- **fix(agy): surface unknown bodiless step vocabulary instead of dropping it (v1.1.13 drift)** — AGY v1.1.13 adds the bodiless `system_message` `step_type` (and emits subagent identity fields plus `workspace_uris` already on the ACTIVE dispatch frame); the parser's enumerated bodiless set silently dropped the new frames. Any bodiless `step_update` whose `step_type` is not `agent_response` (the text step, governed by the B4 usage-carry contract) now yields a non-empty lifecycle event, so future step vocabulary degrades observably. Locked by `tests/test_agy_parser.py::test_v1_1_13_system_message_step_surfaces_as_lifecycle_event`, `...::test_future_unknown_bodiless_step_type_surfaces_as_lifecycle_event`, and `...::test_v1_1_13_subagent_active_entries_carry_identity_and_workspace_uris`; measured capture tracked in `tests/display/_fixtures/agy_wire_provenance.md`.
- **fix(display): keep one live status lifecycle across policy hand-back and commit generation** — normal workspace runs now retain their caller-owned display through policy preflight, remediation, and the resumed pipeline phase, replacing a stale `Policy Remediation` footer immediately; direct agent commit generation now reports `Commit Generation` through the same bounded lifecycle. `f190c1c49`; locked by `tests/project_policy/test_policy_handback_to_pipeline.py`, `tests/project_policy/test_cli_integration_helpers.py`, `tests/test_cli_commit_command.py`, and `tests/integration/test_status_bar_runtime_visibility.py`.
- **fix(claude): allow a bounded 120-second initial-output grace** — `agent_no_output_at_start_seconds` now defaults to 120.0 seconds, accommodates slow Claude initialization, and remains a positive validated `[general]` override that preserves `NO_OUTPUT_AT_START` recovery. Locked by `tests/config/test_no_output_at_start_config.py`.
- **fix(claude): feed the parent transcript parser exactly once per line** — `_transcript_thread` fed each raw parent-transcript line through the same stateful `ClaudeInteractiveTranscriptParser` instance TWICE per loop iteration (once for subagent-tailer routing, again inside `transcript_lines_from_event`); the parser's `session_id` / dedup-signature state had already advanced past the line on the second call, so it silently returned fewer or zero events, starving the ENTIRE operator-facing output stream (no session ID, no tool activity observed) even while the transcript file on disk grew with real content — exactly the live symptom in `smoke-interactive-claude --subagents`. The events from the ONE `feed()` call are now reused for both the subagent-tailer routing and the new pure `transcript_lines_from_events(raw_line, events)` helper. Locked by `tests/agents/invoke/test_transcript_thread_single_feed.py`.
- **fix(claude): exclude pre-existing sibling sessions from transcript discovery** — `find_latest_claude_transcript_entry`'s `min_mtime` floor proved a candidate file was touched since the floor, not created since it, so the orchestrating session's own (unrelated, still-active) transcript in the same `~/.claude/projects/<project-key>` directory could win "latest mtime" over the freshly-spawned child's transcript for the entire run. `run_pty_and_read_lines` now snapshots the workspace's existing transcript names via `existing_transcript_names` BEFORE `spawn_pty` and threads it through `_PtyExtras.pre_existing_transcript_names` to `find_latest_claude_transcript_entry(..., exclude_names=...)`. Locked by `tests/test_pty_transcript_path_resolution.py` and `tests/agents/invoke/test_transcript_discovery_concurrent_session.py`.
- **fix(claude): tail subagent transcripts and route events to the watchdog** — `_subagent_transcript.py` discovers every `agent-*.jsonl` under the parent's `subagents/` directory and forwards parsed events through `record_subagent_work`, replacing the previous in-process-blank assumption that made the watchdog kill productive subagents every ~5 minutes. The parent transcript thread in `_pty_line_reader.py:_transcript_thread` now constructs the tailer lazily on the first observed `session` event (not at reader-thread start, which is before the visible-TUI extractor has populated `_captured_session_id`), calls `note_parent_record` for every parsed record so the Claude Code `version` is captured for the R7 diagnostic, calls `note_dispatch` for every `tool_use:Agent` / `tool_use:Task` block so R7 fires the moment a dispatch lands, and calls `note_completion` for every `tool_result` block so the matching child is dropped on R1 lifecycle ownership. Locked by `tests/agents/invoke/test_subagent_transcript_tail.py`, `tests/agents/invoke/test_pty_transcript_tailer_wiring.py`, and `tests/agents/idle_watchdog/test_no_kill_under_real_subagent_work.py`.
- **fix(claude): classify synthetic and error-envelope assistant turns via shared envelope helper** — `_assistant_envelope.classify_assistant_record` is the single source of truth for the `<synthetic>` model marker and both `isApiErrorMessage` placements (top-level current wire + legacy nested); neither the literal text nor the model name is filtered, so future Claude Code builds that rename the bookkeeping envelope still classify it correctly. Locked by `tests/agents/parsers/test_claude_interactive_transcript_parser.py`, `tests/agents/parsers/test_claude_headless_wire_capture.py`, and `tests/test_claude_interactive_parser.py`.
- **fix(recovery): bound the same-shape retry loop with a configurable ceiling** — `agent_max_same_shape_resumes` (default 3, ge 1) catches the 25-minute / four-cycle burn before it can complete; the fingerprint is `(last_fire_reason, fire_diagnostic_signature, no-new-artifact-since-prior, workspace-change-since-prior)`. Locked by `tests/recovery/test_same_shape_retry_bounds.py` and `tests/config/test_general_config_same_shape_resumes.py`.
- **fix(mcp): serialize concurrent wire-ledger appends with a non-blocking `fcntl.flock`** — two concurrent MCP server instances writing to the same `.agent/tmp/mcp-wire-ledger.jsonl` (the restart-aware bridge tears down a previous server and starts a new one in the same turn) used to leave the chain with two children of the same parent, so `verify_chain` failed at the first non-contiguous row and every run graded below WIRE. Locked by `tests/test_mcp_wire_ledger.py::test_concurrent_appends_chain_safely`.
- **fix(smoke): subagent-contract failures are fatal in the exit code** — the `not every subagent dispatch has a correlated result` and `no meaningful activity was observed after the subagent result` errors MUST surface as a non-zero `EXIT_CODE`, not as a quiet "Observed breaks" cell. The previous "treat as non-fatal" allowlist silently downgraded a missing subagent-tail to `exit_code 0` even when the model raced itself and never produced the required correlated result or post-result activity, defeating the R8 contract that "a transport that cannot dispatch a subagent reports a failed check". With `RALPH_BROKER_SECRET` set, `ralph smoke-interactive-claude --subagents` and `ralph smoke-headless-claude --subagents` report `EXIT_CODE=1` whenever the subagent contract fails; the surfacing in the per-check table is unchanged. Locked by `tests/test_cli_smoke.py::test_subagent_dispatch_missing_is_fatal_exit_code`, `..._test_subagent_no_post_result_activity_is_fatal_exit_code`, `..._test_subagent_dispatch_uncorrelated_returns_nonzero_for_both_transports`, and the live reports under `.agent/tmp/`.
...
- **fix(agy): declare subagent delegation SUPPORTED and remove the stale `agy agents` availability veto** — the canonical `delegation_capabilities` declaration now cites the measured v1.1.10-v1.1.13 stream-json captures (define_subagent / invoke_subagent / manage_subagents with correlated step_update subagent frames), and `effect_router` no longer rejects AGY parallel plans when `agy agents` lists no configured agents. Locked by `tests/test_delegation_capabilities.py` and `tests/test_effect_router_dormant_fanout.py`.
...
- **fix(opencode)!: remove `--pure` and its `pure` option field** — `--pure` runs opencode *without* external plugins, so an operator whose model provider is plugin-provided had every run die on an unresolvable provider. The two production call sites were already gone; the emitter branch in the command builder and the `pure` field threaded through `InvokeOptions`, `InvokeRuntimeOptions`, and `_BuildCommandOptions` are now deleted too, so no caller, config binding, or SDK user can bring it back. Locked by `tests/agents/invoke/test_command_builder_spec.py` and `tests/agents/invoke/test_invoke_dispatch_parity.py`.
...
- **docs(mcp): correct the OpenCode restricted-mode claim in `mcp-tool-restriction.md`** — the page asserted that the provider-visible `mcp` section contains only the Ralph entry. OpenCode's config loader deep-merges `OPENCODE_CONFIG_CONTENT` over the global, project and `.opencode/*` sources rather than replacing them, so the operator's own servers stay in the session; `opencode debug config` shows them alongside `ralph`. That is the intended outcome — not hiding an operator's servers is the same property that made dropping the forced `--pure` restore their plugin-supplied model provider — but the page now says so, and records the real cost as a known limitation: OpenCode is the only transport with no `load_existing_<agent>_upstream_servers`, so those servers are never re-exposed as `ralph_upstream__*` proxies and their tools run outside Ralph's capability gate. No behaviour changed.

- **docs(kimi): record Kimi Code's headless, MCP, and alias gotchas in the add-agent guides** — `docs/agents/adding-a-new-agent.md` gains three common-mistakes entries (stale `kimi-cli` `--print`/`--afk` flags are rejected by the current binary and `--yolo`/`--auto`/`--plan` conflict with `-p`; the workspace `.kimi-code/mcp.json` is silently ignored in untrusted headless folders so MCP wiring targets the user-global `$KIMI_CODE_HOME/mcp.json`; and `kimi/<model>` aliases must carry full configured IDs such as `kimi/kimi-code/k3-256k`), and `docs/agents/quickstart-add-a-new-agent.md` records the same measured headless contract. Locked by `tests/agents/test_doc_adding_a_new_agent.py` and `tests/test_repo_root_agents_docs_sync_current_agents_guides.py`.
- **docs(agy): record the live v1.1.13 `error_message` `step_type`** — both live `smoke-interactive-agy` runs (regular and `--subagents`) emitted a bodiless `step_update` with `"step_type": "error_message"` mid-conversation (not adjacent to the closing frames); the parser's generic bodiless rule already surfaces it as a lifecycle event, so no parser change was needed. Measured vocabulary tracked in `tests/display/_fixtures/agy_wire_provenance.md`.
