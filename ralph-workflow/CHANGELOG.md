...
- Removed session-long native MCP config overlays for AGY, Cursor, and Kimi; each invocation now uses an isolated private config root so concurrent sessions do not serialize on operator-global files.

### Added

- **feat(agy): pass `--print-timeout 2h` on every headless invocation** — AGY's provider-owned print deadline (five minutes) terminated long, valid sessions around 298 s with `{"status":"ERROR","error":"timeout waiting for response"}` even while AGY was actively using MCP tools, and neither the idle watchdog nor MCP activity tracking can extend it. The flag is emitted declaratively via `AgyCommandBuilder.SPEC.extra_flags_before_prompt`, immediately before `--print`; two hours exceeds both the default 90-minute development-timebox hard stop and the 95-minute generic session ceiling, so Ralph Workflow remains the lifecycle authority. Locked by `tests/test_agy_command_builder.py` and `tests/agents/invoke/test_command_builder_spec.py`.
...
- **fix(recovery): synchronize dispatch state and conflict-resolution priority** — dispatch clears stale retry/session state; conflict rounds skip cooling agents, restore priority on expiry, and reset successful resolvers without advancing. Locked by `tests/test_pipeline_runner_invoke_agent_drain_clearing.py` and `tests/test_conflict_resolution_phase_parity.py`.

- **fix(mcp): distinguish inactivity and hard-cap command timeouts** — timeout errors now identify whether output stopped long enough to exhaust the caller's inactivity window (where output resets the window and a larger bounded `timeout_ms` may help) or whether the command reached the non-extendable `EXEC_MAX_TIMEOUT_MS` ceiling (where the agent must split work or run it in the background). The shared execution path keeps `exec`, `unsafe_exec`, and `raw_exec` consistent.
...
- **fix(mcp): isolate native agent configuration per invocation** — AGY, Cursor, and Kimi no longer overwrite user-global MCP files for the duration of an agent session. Their runtime resolvers now build merged configuration under invocation-owned private roots and point each child process at its own root, eliminating cross-process serialization and preserving operator files even across abnormal exits. Locked by `tests/agents/invoke/test_native_mcp_private_root_concurrency.py` and `tests/agents/invoke/test_runtime_resolver_endpoint_delivery.py`.
