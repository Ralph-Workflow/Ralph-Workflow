...
- `external library has no type support` is allowed only when the suppression exactly matches the policy case in `docs/agents/type-ignore-policy.md#external-library`. If the real blocker is first-party Ralph code, the suppression must be removed.
