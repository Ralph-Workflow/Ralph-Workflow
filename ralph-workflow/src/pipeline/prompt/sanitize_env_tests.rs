...
fn test_sanitize_command_env_removes_anthropic_vars_when_not_explicitly_set() {
    ...
    let sanitized = sanitize_command_env(
        env_vars,
        &agent_env_vars,
...
fn test_sanitize_command_env_handles_empty_env_vars() {
    ...
    assert!(sanitized.is_empty(), "Empty environment should remain empty");
}

#[test]
fn test_sanitize_command_env_handles_all_anthropic_vars() {
    // Setup: Environment with all Anthropic vars
    let env_vars: std::collections::HashMap<String, String> = ANTHROPIC_ENV_VARS_TO_SANITIZE
        .iter()
        .map(|&var| (var.to_string(), format!("value-{var}")))
        .chain(std::iter::once(("OTHER_VAR".to_string(), "other-value".to_string())))
        .collect();
