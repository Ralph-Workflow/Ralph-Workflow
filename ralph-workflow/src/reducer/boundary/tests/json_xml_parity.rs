...
#[test]
fn plan_json_and_xml_produce_equivalent_reducer_outcome() {
    ...
    json_ws
        .write_artifact_json(&ArtifactEnvelope::new(
            ...
            serde_json::json!({
                ...
                "steps": [
                    {
                        ...
                        "targets": [
                            {"path": "src/config.rs", "action": "modify"}
                        ],
                ...
                "critical_files": {
                    "primary_files": [
                        {"path": "src/config.rs", "action": "modify", "estimated_changes": "~20 lines"}
                    ]
                },
                "risks_mitigations": [
                    {"severity": "low", "risk": "Breaking existing configuration", "mitigation": "Add backward compatibility"}
                ],
                "verification_strategy": [
                    {"method": "Run unit tests", "expected_outcome": "All tests pass"}
                ]
            }),
    ...
    let xml_ws = MemoryWorkspace::new_test().with_file(
        crate::files::llm_output_extraction::file_based_extraction::paths::PLAN_XML,
        PLAN_XML,
    );
...
#[test]
fn development_result_json_and_xml_produce_equivalent_reducer_outcome() {
    let json_ws = MemoryWorkspace::new_test();
    json_ws
        .write_artifact_json(&ArtifactEnvelope::new(
            "development_result",
    ...
    let xml_ws = MemoryWorkspace::new_test().with_file(
        crate::files::llm_output_extraction::file_based_extraction::paths::DEVELOPMENT_RESULT_XML,
...
#[test]
fn issues_json_and_xml_produce_equivalent_reducer_outcome() {
    let json_ws = MemoryWorkspace::new_test();
    json_ws
        .write_artifact_json(&ArtifactEnvelope::new(
            "issues",
...
#[test]
fn fix_result_json_and_xml_produce_equivalent_reducer_outcome() {
    let json_ws = MemoryWorkspace::new_test();
    json_ws
        .write_artifact_json(&ArtifactEnvelope::new(
            "fix_result",
    ...
    let xml_ws = MemoryWorkspace::new_test().with_file(
        crate::files::llm_output_extraction::file_based_extraction::paths::FIX_RESULT_XML,
        FIX_RESULT_XML,
    );
...
#[test]
fn commit_message_json_and_xml_produce_equivalent_reducer_outcome() {
    let json_ws = MemoryWorkspace::new_test();
    json_ws
        .write_artifact_json(&ArtifactEnvelope::new(
            "commit_message",
...
fn run_plan_parity_case(
    ...
    let json_event = {
        let mut fixture = TestFixture::with_workspace(json_workspace);
        let ctx = fixture.ctx();
    ...
    let xml_event = {
        let mut fixture = TestFixture::with_workspace(xml_workspace);
        let ctx = fixture.ctx();
        let handler = MainEffectHandler::new(state.clone());
        handler
            .validate_planning_xml(&ctx, 0)
...
fn run_issues_parity_case(
    ...
    let json_event = {
        ...
        let ctx = fixture.ctx();
        let handler = MainEffectHandler::new(state.clone());
        handler.validate_review_issues_xml(&ctx, 0).event
    };
...
fn run_commit_parity_case(
    ...
    let json_event = {
        ...
        let ctx = fixture.ctx();
        let handler = MainEffectHandler::new(state.clone());
        handler.validate_commit_xml(&ctx).event
    };
