...
fn dev_fix_completed_does_not_directly_interrupt_when_attempts_exhausted() {
    ...
    let state = {
        ...
        s.dev_fix_attempt_count = 12;
        s.recovery_escalation_level = 4;
...
fn level_2_phase_start_recovery_clears_retry_and_continuation_flags() {
    ...
    let state = {
        ...
        s.continuation.xsd_retry_session_reuse_pending = true;
        s.continuation.same_agent_retry_pending = true;
        s.continuation.same_agent_retry_reason =
            Some(crate::reducer::state::SameAgentRetryReason::Timeout);
        s.continuation.continue_pending = true;
        s.continuation.fix_continue_pending = true;
        s.continuation.context_write_pending = true;
        s.continuation.context_cleanup_pending = true;
        s
    };
...
fn completion_marker_write_failed_sets_pending_flag_for_deterministic_retry() {
    ...
    let state = {
        let mut s = state;
        s.phase = PipelinePhase::AwaitingDevFix;
        s.completion_marker_pending = false;
        s
    };
...
fn completion_marker_emitted_records_is_failure_and_clears_pending_state() {
    ...
    let state = {
        ...
        s.completion_marker_reason = Some("previous error".to_string());
        s
    };
