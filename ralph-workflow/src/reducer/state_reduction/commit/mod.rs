...
fn post_commit_review_issues_flag(
    state: &PipelineState,
    next_phase: crate::reducer::event::PipelinePhase,
) -> bool {
    if state.previous_phase == Some(crate::reducer::event::PipelinePhase::Development)
        && next_phase == crate::reducer::event::PipelinePhase::FinalValidation
    ...
    match event {
        ...
        CommitEvent::Created { hash, .. } => {
            ...
            PipelineState {
                ...
                review_issues_found: post_commit_review_issues_flag(&state, next_phase),
                context_cleaned: false,
                commit_required_files_cleaned: false,
                commit_diff_prepared: false,
                commit_diff_empty: false,
...
fn reduce_residual_files_found(
    ...
    if let Some(resume_phase) = base.termination_resume_phase {
        ...
        PipelineState {
            ...
            iteration: next_iter,
            reviewer_pass: next_reviewer_pass,
            review_issues_found: post_commit_review_issues_flag(&base, next_phase),
            context_cleaned: false,
            commit_required_files_cleaned: false,
            commit_diff_prepared: false,
            commit_diff_empty: false,
...
fn reduce_residual_files_none(state: PipelineState) -> PipelineState {
    let final_pass = 1u8.saturating_add(state.max_commit_residual_retries);
    if state.commit_residual_retry_pass > final_pass {
        let in_recovery_loop = state.phase == crate::reducer::event::PipelinePhase::AwaitingDevFix
            || state.previous_phase == Some(crate::reducer::event::PipelinePhase::AwaitingDevFix);
