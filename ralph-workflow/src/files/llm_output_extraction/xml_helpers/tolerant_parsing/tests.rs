...
use crate::files::llm_output_extraction::xml_helpers::tolerant_parsing::{
    normalize_enum_value, normalize_tag_name, DEVELOPMENT_STATUS_SYNONYMS, FILE_ACTION_SYNONYMS,
    FIX_STATUS_SYNONYMS, LIST_TYPE_SYNONYMS, PRIORITY_SYNONYMS, SEVERITY_SYNONYMS,
    STEP_TYPE_SYNONYMS,
};
...
fn test_development_empty_string_returns_none() {
    let result = normalize_enum_value("", DEVELOPMENT_VALID_VALUES, DEVELOPMENT_STATUS_SYNONYMS);
    assert_eq!(result, None);
}
...
fn test_step_type_exact_match_file_change() {
    let result = normalize_enum_value("file-change", STEP_TYPE_VALID_VALUES, STEP_TYPE_SYNONYMS);
...
fn test_step_type_synonym_code_change_maps_to_file_change() {
    let result = normalize_enum_value("code-change", STEP_TYPE_VALID_VALUES, STEP_TYPE_SYNONYMS);
    assert_eq!(result, Some("file-change".to_string()));
}

#[test]
fn test_step_type_synonym_implementation_maps_to_file_change() {
    let result = normalize_enum_value("implementation", STEP_TYPE_VALID_VALUES, STEP_TYPE_SYNONYMS);
    assert_eq!(result, Some("file-change".to_string()));
}

#[test]
fn test_step_type_synonym_investigate_maps_to_research() {
    let result = normalize_enum_value("investigate", STEP_TYPE_VALID_VALUES, STEP_TYPE_SYNONYMS);
    assert_eq!(result, Some("research".to_string()));
}
