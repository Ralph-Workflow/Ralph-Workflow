...
pub(crate) fn track_file_path_for_repo_root(repo_root: &Path) -> PathBuf {
    ralph_git_dir(repo_root).join(WRAPPER_TRACK_FILE_NAME)
}
...
pub(crate) fn cleanup_stray_tmp_files_from_repo(repo_root: &Path) {
...
pub(crate) fn track_file_exists_at_path(path: &Path) -> bool {
    fs::symlink_metadata(path)
        .is_ok_and(|meta| meta.file_type().is_file() && !meta.file_type().is_symlink())
}

pub(crate) fn remove_track_file(track_path: &Path) {
    #[cfg(unix)]
    add_owner_write_if_not_symlink_for_track(track_path);
    let _ = fs::remove_file(track_path);
}

#[cfg(unix)]
fn add_owner_write_if_not_symlink_for_track(path: &Path) {
    use std::os::unix::fs::PermissionsExt;
    if matches!(
        fs::symlink_metadata(path),
