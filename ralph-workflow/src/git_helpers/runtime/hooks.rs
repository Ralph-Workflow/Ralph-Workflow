...
use crate::git_helpers::repo::{resolve_protection_scope_from, ProtectionScope};
...
pub fn verify_hooks_removed(repo_root: &Path) -> io::Result<Vec<&'static str>> {
    let hooks_dir = crate::git_helpers::repo::get_hooks_dir_from(repo_root)?;
    if !hooks_dir.exists() {
        return Ok(Vec::new());
    }
...
pub fn enforce_hook_permissions(repo_root: &Path, logger: &Logger) {
    ...
    let Ok(hooks_dir) = crate::git_helpers::repo::get_hooks_dir_from(repo_root) else {
        return;
    };
