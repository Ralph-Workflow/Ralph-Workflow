...
use crate::git_helpers::repo::{get_repo_root, normalize_protection_scope_path};
use crate::git_helpers::runtime::hooks::{reinstall_hooks_if_tampered, uninstall_hooks_silent_at};
...
fn repair_marker_path_if_tampered(repo_root: &Path) -> io::Result<()> {
    let ralph_dir = crate::git_helpers::repo::ralph_git_dir(repo_root);
    ...
    if let Ok(meta) = fs::symlink_metadata(&marker_path) {
        ...
        if !is_regular_file {
            crate::git_helpers::repo::quarantine_path_in_place(&marker_path, "marker")?;
        }
    }
...
fn enable_git_wrapper_at(repo_root: &Path, helpers: &mut GitHelpers) -> io::Result<()> {
    ...
    let scope = crate::git_helpers::repo::resolve_protection_scope_from(repo_root)?;
...
pub fn disable_git_wrapper(helpers: &mut GitHelpers) {
    ...
    let track_file = repo_root.as_ref().map_or_else(
        ...
        |r| crate::git_helpers::repo::ralph_git_dir(r).join(WRAPPER_TRACK_FILE_NAME),
    );
...
pub fn start_agent_phase_in_repo(repo_root: &Path, helpers: &mut GitHelpers) -> io::Result<()> {
    ...
    if let Ok(hooks_dir) = crate::git_helpers::repo::get_hooks_dir_from(repo_root) {
    ...
    crate::git_helpers::runtime::hooks::install_hooks_in_repo(repo_root)?;
...
pub fn ensure_agent_phase_protections(logger: &Logger) -> ProtectionCheckResult {
    ...
    let Ok(scope) = crate::git_helpers::repo::resolve_protection_scope() else {
        return result;
    };
    ...
    let hooks_present = crate::git_helpers::repo::get_hooks_dir_from(&repo_root)
        .ok()
        .is_some_and(|hooks_dir| {
            crate::git_helpers::runtime::hooks::RALPH_HOOK_NAMES
                .iter()
                .any(|name| {
                    ...
                    path.exists()
                        && matches!(
                            crate::files::file_contains_marker(
                                &path,
                                crate::git_helpers::runtime::hooks::HOOK_MARKER
                            ),
                            Ok(true)
                        )
                })
    ...
    #[cfg(unix)]
    crate::git_helpers::runtime::hooks::enforce_hook_permissions(&repo_root, logger);
...
fn cleanup_prior_wrapper_from_track_file(repo_root: &Path) {
    let ralph_dir = crate::git_helpers::repo::ralph_git_dir(repo_root);
    let Ok(ralph_dir_exists) = crate::git_helpers::repo::sanitize_ralph_git_dir_at(&ralph_dir)
    else {
        return;
    };
    if !ralph_dir_exists {
...
pub fn verify_wrapper_cleaned(repo_root: &Path) -> Vec<String> {
    ...
    let track_file =
        crate::git_helpers::repo::ralph_git_dir(repo_root).join(WRAPPER_TRACK_FILE_NAME);
...
fn cleanup_agent_phase_protections_silent_at_internal(
    ...
    let ralph_dir = if let Some(ralph_dir) = stored_ralph_dir {
        ...
        computed_ralph_dir = crate::git_helpers::repo::ralph_git_dir(repo_root);
        &computed_ralph_dir
    };
...
pub fn cleanup_orphaned_marker(logger: &Logger) -> io::Result<()> {
    ...
    let ralph_dir = crate::git_helpers::repo::ralph_git_dir(&repo_root);
    if !crate::git_helpers::repo::sanitize_ralph_git_dir_at(&ralph_dir)? {
...
pub fn detect_unauthorized_commit(repo_root: &Path) -> bool {
    let head_oid_path = crate::git_helpers::repo::ralph_git_dir(repo_root).join(HEAD_OID_FILE_NAME);
    if matches!(fs::symlink_metadata(&head_oid_path), Ok(m) if m.file_type().is_symlink()) {
        return false;
    }
...
mod tests {
    ...
    fn test_cleanup_agent_phase_silent_at_removes_ralph_hooks() {
        ...
        use crate::git_helpers::runtime::hooks;
    ...
    fn test_cleanup_agent_phase_silent_at_from_worktree_root() {
        ...
        crate::git_helpers::runtime::hooks::install_hooks_in_repo(&wt_path).unwrap();
        ...
        for name in crate::git_helpers::runtime::hooks::RALPH_HOOK_NAMES {
    ...
    fn test_cleanup_agent_phase_silent_at_from_root_repo_restores_root_worktree_scoping() {
        ...
        crate::git_helpers::runtime::hooks::install_hooks_in_repo(&main_repo_path).unwrap();
        assert!(
            root_config.exists(),
