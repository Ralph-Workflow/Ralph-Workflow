...
fn list_child_pids_inner(
    ...
    list_child_pids_process_result(bytes_written, buffer, capacity)
}

fn list_child_pids_process_result(
    bytes_written: i32,
    buffer: Vec<libc::pid_t>,
    capacity: usize,
) -> Option<(Vec<u32>, Option<usize>)> {
    if bytes_written == 0 {
    ...
    let next_capacity = capacity.checked_mul(2)?;
    Some((Vec::new(), Some(next_capacity)))
}
...
fn compute_libproc_child_info(parent_pid: u32, descendants: &[u32]) -> Option<ChildProcessInfo> {
    let entries: Vec<_> = descendants
        .iter()
        .filter_map(|descendant_pid| {
            let bsd_info = fetch_bsd_short_info(*descendant_pid)?;
            ...
            Some(LibprocDescendantEntry {
                pid: *descendant_pid,
                cpu_time_ms,
                is_currently_active: libproc_state_indicates_current_activity(
                    bsd_info.status,
                    cpu_time_ms,
                    num_running_threads,
                ),
    ...
    let child_count = u32::try_from(entries.len()).unwrap_or(u32::MAX);
    let active_child_count = entries.iter().filter(|e| e.is_currently_active).count() as u32;
    let total_cpu_ms = entries.iter().map(|e| e.cpu_time_ms).sum();
    let qualifying_pids: Vec<u32> = entries.iter().map(|e| e.pid).collect();

    Some(ChildProcessInfo {
        child_count,
        active_child_count,
        cpu_time_ms: total_cpu_ms,
        descendant_pid_signature: descendant_pid_signature(&qualifying_pids),
    })
}

struct LibprocDescendantEntry {
    pid: u32,
    cpu_time_ms: u64,
