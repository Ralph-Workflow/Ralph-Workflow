...
- **Bounded auto-restart** — if the MCP server becomes unhealthy during a pipeline run, the bridge restarts it automatically up to `McpRestartPolicy.max_restarts` times (default 3). When the budget is exhausted, `McpServerError` is raised and the pipeline surfaces the restart count so the failure is diagnosable.
