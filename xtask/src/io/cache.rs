...
const DYLINT_SCOPE_GLOBS: &[ScopeGlob] = &[
    ...
    ScopeGlob {
        dir: "lints/forbid_mut_binding/src",
        pattern: "*.rs",
    },
    ScopeGlob {
        dir: "lints/forbid_imperative_loops/src",
        pattern: "*.rs",
    },
    ScopeGlob {
        dir: "lints/forbid_mutating_receiver_methods/src",
        pattern: "*.rs",
    },
    ScopeGlob {
        dir: "lints/forbid_interior_mutability/src",
        pattern: "*.rs",
    },
];
...
fn append_native_scan_definition_hash(
    ...
    for check in checks {
        ...
        match check.mode {
            crate::io::scanner::MatchMode::AnyLiteral { skip_comment_lines } => {
                hasher.write_bytes(b"any-literal");
                hasher.write_bytes(&[u8::from(skip_comment_lines)]);
            }
            crate::io::scanner::MatchMode::StemWithBoolSuffix => {
                hasher.write_bytes(b"stem-with-bool-suffix");
            }
            crate::io::scanner::MatchMode::AnyLiteralAtLineStart { skip_comment_lines } => {
                hasher.write_bytes(b"any-literal-at-line-start");
                hasher.write_bytes(&[u8::from(skip_comment_lines)]);
            }
            crate::io::scanner::MatchMode::NegativeLookahead {
                negative_context,
...
impl CommandRunner for CachingCommandRunner {
    ...
    fn run_native_check(
...

        {
            let memory = self.memory.lock().unwrap();
            if memory.contains_key(&key) {
                return Ok(crate::runtime::verify::NativeCheckResult {
        ...
        if result.status == crate::runtime::verify::CheckStatus::Pass {
            self.memory.lock().unwrap().insert(
                key,
                CacheEntry {
                    scope_hash: hash,
...
mod tests {
    ...
    fn counted_native_required_check(
        repo_root: &Path,
    ) -> crate::runtime::verify::NativeCheckResult {
        ...
        if sentinel.exists() {
            ...
            crate::runtime::verify::NativeCheckResult {
                status: crate::runtime::verify::CheckStatus::Warning,
                message: "missing sentinel".to_string(),
            }
        }
    }

    fn native_required_check(name: &'static str) -> crate::runtime::verify::NativeCheck {
        crate::runtime::verify::NativeCheck {
            name,
...
"#,
    ...
    fn test_native_scan_hash_ignores_excluded_transient_frontend_files() {
        ...
        let check = crate::io::scanner::NativeScanCheck {
            ...
            mode: crate::io::scanner::MatchMode::AnyLiteralAtLineStart {
    ...
    fn test_native_required_check_clean_result_is_cached_for_unchanged_run() {
        ...
        assert_eq!(first.status, crate::runtime::verify::CheckStatus::Pass);
        assert_eq!(second.status, crate::runtime::verify::CheckStatus::Pass);
        assert_eq!(
