...
def test_generate_commit_stages_working_tree_changes_when_nothing_is_staged(
    ...
    monkeypatch.setattr(
        commit_module,
        "invoke_agent",
        lambda *_a, **_k: iter(['{"type":"text","text":"fix: generated by agent"}']),
    )
...
def test_generate_commit_uses_commit_drain_agent_chain(monkeypatch: pytest.MonkeyPatch) -> None:
    stream = _attach_console(monkeypatch, commit_module)
    monkeypatch.setattr(commit_module, "find_repo_root", lambda: Path("/tmp"))
    monkeypatch.setattr(commit_module, "load_config", lambda *_: _simple_config())
    monkeypatch.setattr(
    ...
    def fake_invoke_agent(agent_config, *_args, **_kwargs):
        ...
        return iter(['{"type":"text","text":"fix: commit drain message"}'])
...
def test_generate_commit_uses_direct_opencode_model_from_commit_drain(
    ...
    monkeypatch.setattr(
        ...
        lambda _name: SimpleNamespace(
            parse=lambda _raw_lines: [
                SimpleNamespace(type="text", content="fix: commit drain message")
            ]
        ),
    )
...
def test_generate_commit_falls_back_to_review_chain_when_commit_chain_unusable(
    ...
    def fake_invoke_agent(agent_config, *_args, **_kwargs):
        ...
        return iter(['{"type":"text","text":"fix: review fallback message"}'])
    ...
    monkeypatch.setattr(
        ...
        lambda _name: SimpleNamespace(
            parse=lambda _raw_lines: [
                SimpleNamespace(type="text", content="fix: review fallback message")
            ]
        ),
    )
...
def test_generate_commit_msg_writes_commit_message_artifact(
    ...
    monkeypatch.setattr(
        commit_module,
        "invoke_agent",
        lambda *_a, **_k: iter(['{"type":"text","text":"feat: persist generated commit message"}']),
    )
...
def test_show_commit_msg_reads_artifact_without_staged_changes(
    ...
    commit_file.write_text("fix: read stored commit message", encoding="utf-8")
...
def test_generate_commit_msg_skip_deletes_existing_artifact(
    ...
    monkeypatch.setattr(
        commit_module,
        "invoke_agent",
        lambda *_a, **_k: iter(['{"type":"text","text":"SKIP: No commit needed"}']),
    )
