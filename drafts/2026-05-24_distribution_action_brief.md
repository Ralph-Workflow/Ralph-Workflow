...
- 21 directory submissions already shipped in the last 24 hours.
- 37 curator contact attempts already shipped in the last 24 hours.
...
- Fresh publisher outreach already shipped in the current 7-day review window (AXME Code, WyeWorks), so those targets should not be re-queued immediately.
- Primary-repo-flat repair already surfaced fresh developer-native publishers with public contact paths (ctxt.dev / Signum, Bollwerk / Werkstatt), so the loop should package that Codeberg-first outreach instead of ending at measurement hold.
