...
//! - `std::net::*`, `reqwest::*`, `ureq::*` — network access
//! - `std::thread::*`, `tokio::task::*`, `tokio::time::*` — thread/task effects
//! - `rand::*`, `getrandom::*` — randomness
//! - `std::io::stdin/stdout/stderr` — stdio handle access
...
declare_lint! {
    ...
    /// - Network access (`std::net`, `reqwest`, `ureq`)
    /// - Threading/task effects (`std::thread`, `tokio::task`, `tokio::time`)
    /// - Randomness (`rand`, `getrandom`)
    /// - Stdio handle access (`std::io::stdin`, `stdout`, `stderr`)
    ...
    Warn,
    "I/O effects (filesystem, env, process, network, threads, randomness, stdio, clock) are forbidden outside boundary modules"
}
...
impl EffectKind {
    const fn description(self) -> &'static str {
        match self {
            ...
            Self::Thread => "thread or async runtime operation",
            Self::Random => "randomness operation",
            Self::Stdio => "stdio handle access",
    ...
    const fn style_guide_rule(self) -> &'static str {
        match self {
            ...
            Self::Network => "perform network I/O",
            Self::Thread => "spawn threads/tasks or sleep in domain code",
            Self::Random => "source randomness in domain code",
            Self::Stdio => "reach into stdin/stdout/stderr",
...
const EFFECT_PATTERNS: &[(&[&str], EffectKind)] = &[
    ...
    (&["std", "net"], EffectKind::Network),
    (&["reqwest"], EffectKind::Network),
    (&["ureq"], EffectKind::Network),
    (&["std", "thread"], EffectKind::Thread),
    (&["tokio", "task"], EffectKind::Thread),
    (&["tokio", "time"], EffectKind::Thread),
    (&["tokio", "runtime"], EffectKind::Thread),
    (&["rand"], EffectKind::Random),
    (&["getrandom"], EffectKind::Random),
];

const STDIO_FUNCTIONS: &[&str] = &[
    "std::io::stdio::stdin",
    "std::io::stdio::stdout",
    "std::io::stdio::stderr",
];

const STDIO_TYPES: &[&str] = &[
    "std::io::Stdin",
    "std::io::Stdout",
    "std::io::Stderr",
    "std::io::StdinLock",
...
fn path_matches_effect(def_path: &str) -> Option<EffectKind> {
    if STDIO_FUNCTIONS.iter().any(|path| def_path == *path) {
        return Some(EffectKind::Stdio);
    }
...
fn is_stdio_effect(cx: &LateContext<'_>, expr: &Expr<'_>) -> Option<EffectKind> {
    ...
    STDIO_TYPES
        .iter()
        .any(|stdio_type| ty_str.starts_with(stdio_type))
        .then_some(EffectKind::Stdio)
}
...
fn emit_diagnostic(cx: &LateContext<'_>, expr: &Expr<'_>, kind: EffectKind, path: &str) {
    cx.span_lint(FORBID_IO_EFFECTS, expr.span, |diag| {
        ...
        diag.note(
            "style guides: `docs/code-style/boundaries.md` and \
...
impl<'tcx> LateLintPass<'tcx> for ForbidIoEffects {
    fn check_expr(&mut self, cx: &LateContext<'tcx>, expr: &'tcx Expr<'tcx>) {
        ...
        if let Some(kind) = is_stdio_effect(cx, expr) {
            emit_diagnostic(cx, expr, kind, "std::io::stdin/stdout/stderr");
            return;
        }
...
mod tests {
    ...
    #[test]
    fn detects_network_effects() {
        assert!(matches!(
            path_matches_effect("std::net::TcpStream::connect"),
            Some(EffectKind::Network)
        ));
        assert!(matches!(
            path_matches_effect("reqwest::blocking::get"),
            Some(EffectKind::Network)
        ));
        assert!(matches!(
            path_matches_effect("ureq::Agent::get"),
            Some(EffectKind::Network)
        ));
    }

    #[test]
    fn detects_stdio_effects() {
        assert!(matches!(
            path_matches_effect("std::io::stdio::stdin"),
            Some(EffectKind::Stdio)
        ));
        assert!(matches!(
            path_matches_effect("std::io::stdio::stdout"),
            Some(EffectKind::Stdio)
        ));
        assert!(matches!(
            path_matches_effect("std::io::stdio::stderr"),
            Some(EffectKind::Stdio)
        ));
    }

    #[test]
    fn detects_thread_and_async_runtime_effects() {
        assert!(matches!(
            path_matches_effect("std::thread::spawn"),
            Some(EffectKind::Thread)
        ));
        assert!(matches!(
            path_matches_effect("std::thread::sleep"),
            Some(EffectKind::Thread)
        ));
        assert!(matches!(
            path_matches_effect("tokio::task::spawn"),
            Some(EffectKind::Thread)
        ));
        assert!(matches!(
            path_matches_effect("tokio::time::sleep"),
            Some(EffectKind::Thread)
        ));
    }

    #[test]
    fn detects_randomness_effects() {
        assert!(matches!(
            path_matches_effect("rand::random"),
            Some(EffectKind::Random)
        ));
        assert!(matches!(
            path_matches_effect("rand::rng"),
            Some(EffectKind::Random)
        ));
        assert!(matches!(
            path_matches_effect("getrandom::fill"),
            Some(EffectKind::Random)
        ));
    }
