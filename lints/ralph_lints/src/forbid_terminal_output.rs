...
/// Terminal output macros that are forbidden in domain code.
...
/// Extract the macro name if it's a terminal output macro.
...
mod tests {
    ...
    fn dbg_is_not_a_terminal_macro() {
        // dbg! goes to stderr but is for debugging, not terminal output
