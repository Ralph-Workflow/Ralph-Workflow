...
Within those modules, the preferred style is explicit and composable. `docs/code-style/coding-patterns.md` favors plain Rust with pure helpers, immutable transformations, and iterator/combinator pipelines over hand-written imperative control flow. Use comments to explain why, not what. Use named constants instead of magic numbers. Prefer straightforward naming and obvious control flow over clever abstractions or compact tricks. When a specific imperative pattern needs to be rewritten to satisfy the functional lints, `docs/code-style/functional-transformations.md` provides a production-level cookbook: collection building without mutation, loop-to-iterator replacements, struct update patterns, conditional transformation chains, and a quick-reference table of imperative-to-functional rewrites.
