...
DEFAULT_COOLDOWN_HOURS = {
    ...
    "github-discussions": 6.0,    # search-and-draft, once per review cycle
    "github-discussions-search": 6.0,
...
SPIDERING_THRESHOLD = 3  # consecutive attempts within cooldown = spidering
...
def guard_check(channel: str) -> tuple[bool, str, float]:
    ...
    if last_attempt_ts:
        ...
        if elapsed < cooldown:
            ...
            # Check for spidering pattern
            recent_count = channel_state.get("recent_cooldown_hits", 0) + 1
            if recent_count >= SPIDERING_THRESHOLD:
                _write_stop_file(
                    channel,
                    f"SPIDERING DETECTED: {recent_count} consecutive attempts within {cooldown}h "
                    f"cooldown window. Auto-perma-blocked to prevent log spam."
                )
                return False, f"spidering_auto_blocked: {recent_count} rapid attempts", float("inf")
            channel_state["recent_cooldown_hits"] = recent_count
            _save_state(state)
            return False, f"cooldown_active", remaining

    # Cooldown expired — reset spidering counter
    if last_attempt_ts:
        channel_state["recent_cooldown_hits"] = 0
        _save_state(state)
...
def guard_record(channel: str, ok: bool, fingerprint: str = "", **kwargs) -> None:
    ...
    # If this attempt was successful and not in cooldown, reset spidering counter
    if ok:
        channel_state["recent_cooldown_hits"] = 0
