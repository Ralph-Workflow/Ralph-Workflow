...
class DistributionLaneSelectorTests(unittest.TestCase):
    ...
    def test_prefers_apollo_launch_handoff_when_apollo_followup_is_due(self):
        now = datetime(2026, 5, 26, 4, 20, 0)
        adoption = {"evaluation": {"failing_signals": ["primary_repo_flat"]}}
        channel_log = {"working": []}
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            reddit_monitor_path.write_text('Partial visibility only. Fail closed. reddit_direct_access_degraded=1\n', encoding='utf-8')
            (drafts_dir / 'apollo_launch_handoff_packet_latest.md').write_text('# Apollo launch handoff packet\n', encoding='utf-8')
            ...
            self.assertEqual(decision.lane, 'apollo_launch_handoff_packet')
            self.assertIn('first review checkpoint', decision.reason)
...
class DistributionLaneSelectorFallbackTests(unittest.TestCase):
    def test_prefers_directory_confirmation_when_live_secondary_surface_repair_packet_is_current(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            channel_path.write_text(json.dumps(channel_log), encoding='utf-8')
            outreach_path.write_text('', encoding='utf-8')
            apollo_path.write_text(json.dumps({'status': 'login_succeeded'}), encoding='utf-8')
            apollo_sequence_path.write_text(json.dumps({'measurement_pending': True, 'next_review_at': '2026-05-30T00:14:49.075391+02:00'}), encoding='utf-8')
            ...
            (log_dir / 'marketing_recent_a.json').write_text(json.dumps({
                'timestamp': '2026-05-25T16:30:00',
                'chosen_action': {'type': 'publisher_email_outreach', 'title': 'A'},
                'result': {'ok': True, 'status': 'executed', 'live_external_action': True},
            }), encoding='utf-8')
            (log_dir / 'marketing_recent_b.json').write_text(json.dumps({
                'timestamp': '2026-05-25T17:00:00',
                'chosen_action': {'type': 'curator_email_outreach', 'title': 'B'},
                'result': {'ok': True, 'status': 'executed', 'live_external_action': True},
            }), encoding='utf-8')
            (log_dir / 'marketing_recent_c.json').write_text(json.dumps({
                'timestamp': '2026-05-25T18:00:00',
                'chosen_action': {'type': 'directory_confirmation_execution', 'title': 'C'},
                'result': {'ok': True, 'status': 'executed', 'live_external_action': False},
            }), encoding='utf-8')
            ...
            with ExitStack() as stack:
                for patcher in [
                    ...
                    patch.object(distribution_lane_selector, '_apollo_sequence_measurement_status', return_value={'measurement_pending': True, 'next_review_at': '2026-05-30T00:14:49.075391+02:00'}),
                    patch.object(distribution_lane_selector, '_live_curator_queue_count', return_value=0),
                    patch.object(distribution_lane_selector, '_prepared_curator_target_names', return_value=[]),
                    patch.object(distribution_lane_selector, '_prepared_curator_targets_waiting_for_handoff', return_value=0),
                    patch.object(distribution_lane_selector, '_curator_handoff_packet_current', return_value=False),
    ...
    def _build_stackoverflow_repeat_guard_fixture(self, *, now: datetime, extra_logs: list[tuple[str, dict]], recent_proof_asset_shipped: bool = False):
        adoption = {"evaluation": {"failing_signals": ["primary_repo_flat"]}}
        channel_log = {"working": []}
        outreach_text = 'HN/Lobsters blocker noted. HN/Lobsters blocker noted. HN/Lobsters blocker noted.'
        curator_queue = {
        ...
        (seo_dir / 'reddit_monitor_latest.md').write_text('Reddit is IP-blocked from this environment.\n**Search diagnostics:** ok=0, reddit_ip_blocked=5\n', encoding='utf-8')
        for filename, payload in extra_logs:
        ...
        patches = [
            ...
            patch.object(distribution_lane_selector, 'REDDIT_MONITOR_LATEST', seo_dir / 'reddit_monitor_latest.md'),
            patch.object(distribution_lane_selector, '_github_auth_available', return_value=False),
            patch.object(distribution_lane_selector, '_recent_executed_action_type', return_value=recent_proof_asset_shipped),
        ]
        return tmpdir_ctx, patches
    ...
    def test_avoids_repeating_stackoverflow_handoff_after_manual_delivery(self):
        ...
        tmpdir_ctx, patches = self._build_stackoverflow_repeat_guard_fixture(
            now=now,
            extra_logs=[(
                'marketing_2026-05-23_stackoverflow_manual_delivery.json',
                {
                    'timestamp': '2026-05-23T20:39:00+02:00',
                    'chosen_action': {'type': 'stackoverflow_manual_delivery'},
                    'result': {'status': 'executed', 'ok': True, 'live_external_action': False},
            ...
            recent_proof_asset_shipped=False,
        )
        with tmpdir_ctx:
    ...
    def test_avoids_repeating_stackoverflow_handoff_when_post_cooldown_run_is_scheduled(self):
        now = datetime(2026, 5, 24, 10, 25, 0)
        tmpdir_ctx, patches = self._build_stackoverflow_repeat_guard_fixture(
            now=now,
            extra_logs=[(
...
class MarketingWorkflowAuditBurstTests(unittest.TestCase):
    ...
    def test_detects_manual_only_block_when_comparison_queue_is_full_and_github_auth_is_missing(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.object(distribution_lane_selector, 'LOG_DIR', log_dir), \
                 ...
                 patch.object(distribution_lane_selector, 'MARKET_INTELLIGENCE_PATH', market_path), \
                 patch.object(distribution_lane_selector, '_github_auth_available', return_value=False):
                blocked = distribution_lane_selector._comparison_backlink_lane_manual_only_blocked(now)
...
class DistributionLaneExecutorTests(unittest.TestCase):
    ...
    def test_directory_confirmation_execution_surfaces_secondary_surface_repairs(self):
        now = datetime(2026, 5, 25, 20, 55, 0)
        decision = distribution_lane_selector.LaneDecision(
            lane='directory_confirmation',
            reason='test',
            reasons=['test'],
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            backlink_status_path = log_dir / 'backlink_status_latest.json'
            backlink_status_path.write_text(json.dumps({
                'generated_at': '2026-05-25T18:08:11+00:00',
                'directories': {
                    'SaaSHub': {
        ...
        self.assertIn('Live secondary surfaces that still need Codeberg-routing repair', text)
        self.assertIn('https://www.saashub.com/ralph-workflow-alternatives', text)
        self.assertIn('Suggested manual repair ask', text)

    def test_marketing_execution_board_surfaces_directory_secondary_surface_packet(self):
        now = datetime(2026, 5, 25, 20, 56, 0)
        ...
        self.assertIn('Directory secondary-surface repair packet', text)
        self.assertIn('SaaSHub', text)

    def test_marketing_execution_board_hides_directory_secondary_surface_packet_during_active_followthrough_window(self):
        now = datetime(2026, 5, 26, 1, 37, 0)
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                'directories': {
                    'SaaSHub': {
                        ...
                        'secondary_surface_targets': [
                            ...
                            {
                                'url': 'https://www.saashub.com/best-workflow-automation-software',
            ...
            (log_dir / 'marketing_2026-05-25_saashub_secondary_surface_execution.json').write_text(json.dumps({
                'timestamp': '2026-05-25T21:05:28+02:00',
                'chosen_action': {
                    'type': 'saashub_secondary_surface_execution',
                    'channel': 'directory_confirmation',
                },
                'targets': [
                    'https://www.saashub.com/ralph-workflow-alternatives',
                    'https://www.saashub.com/best-workflow-automation-software',
                ...
                'review_window': {
                    'follow_up_not_before': '2026-05-31T00:00:00+02:00',
                },
            }), encoding='utf-8')
...
class MarketingLoopCertificationTests(unittest.TestCase):
    ...
    def test_independent_verifier_flags_stale_outcome_execution_board_status_during_hold(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            os.utime(outcome_status, (stale_ts, stale_ts))
        ...
        self.assertFalse(ok)
        self.assertTrue(any('outcome_execution_board_latest.json is stale' in blocker for blocker in blockers))
        self.assertEqual(watchpoints, [])

    def test_independent_verifier_accepts_fresh_outcome_execution_board_status_during_hold(self):
        now = datetime(2026, 5, 26, 23, 30, 0)
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            runner = tmp / 'runner.json'
