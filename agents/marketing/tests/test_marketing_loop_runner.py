...
class MarketingLoopRunnerTests(unittest.TestCase):
    def test_audit_requires_post_audit_runtime_only_for_supported_follow_through_states(self):
        ...
        self.assertFalse(marketing_loop_runner._audit_requires_post_audit_runtime({
            'repair_window_status': 'measurement_pending',
            'measurement_pending_reasons': ['same_family_publisher_overlap'],
            'repair_actions': [
                {
                    'repair_kind': 'system_design',
                    'repair_state': 'pending_measurement',
    ...
    def test_main_triggers_post_audit_runtimes_when_audit_opens_system_design_repair(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            post_entries = [entry for entry in payload['results'] if entry.get('triggered_by') == marketing_loop_runner.POST_AUDIT_TRIGGER_SYSTEM_DESIGN_REPAIR]
            self.assertEqual(len(post_entries), 2)

    def test_main_runs_execution_board_follow_through_when_audit_is_measurement_pending_for_primary_repo_flat(self):
        audit_stdout = json.dumps({
            'repair_window_status': 'measurement_pending',
            'measurement_pending_reasons': ['primary_repo_flat'],
            'repair_actions': [
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn('outcome_execution_board_runner.py', scripts)
            post_entries = [entry for entry in payload['results'] if entry.get('triggered_by') == marketing_loop_runner.POST_AUDIT_TRIGGER_MEASUREMENT_PENDING]
            self.assertEqual(len(post_entries), 1)

    def test_main_skips_post_audit_runtimes_when_measurement_pending_lacks_primary_repo_flat_reason(self):
        audit_stdout = json.dumps({
            'repair_window_status': 'measurement_pending',
            'measurement_pending_reasons': ['same_family_publisher_overlap'],
            'repair_actions': [
