...
mypy provides strict static type checking with Pydantic integration. The maintained Python package requires strict type checking:
...
- First-party fixes come first: prefer a local annotation, guard, typed helper, or `cast(...)` before keeping a suppression
