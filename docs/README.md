# Ralph Workflow Docs

Start here if you already know Ralph Workflow is interesting and want the shortest path to the right page.

## Best first clicks

- [Start here on one real task](../START_HERE.md)
- [First-task guide](./first-task-guide.md)
- [Workflow composition example](../content/examples/workflow_composition_example.md)
- [Tomorrow-morning scorecard](../content/examples/tomorrow_morning_scorecard.md)
- [Example review bundle](../content/examples/review_bundle_example.md)
- [When to use Ralph Workflow](./when-to-use-ralph-workflow.md)
- [Review AI coding output before merge](../content/guides/review_ai_coding_output_before_merge.md)
- [Good unattended task vs bad one](../content/guides/good_unattended_task.md)

## Guides

- [Unattended AI coding workflow](../content/guides/unattended_ai_coding_workflow.md)
- [Autonomous AI workflows for production reliability](../content/guides/autonomous_ai_workflows_production_reliability.md)
- [Spec-driven AI agent](../content/guides/spec_driven_ai_agent.md)
- [Claude Code + Codex workflow](../content/guides/claude_code_codex_workflow.md)

## Comparisons

- [Ralph Workflow vs Claude Code](./ralph-workflow-vs-claude-code.md)
- [Ralph Workflow vs Aider](./ralph-workflow-vs-aider.md)
- [Ralph Workflow vs Cursor](./ralph-workflow-vs-cursor.md)
- [Ralph Workflow vs Continue](./ralph-workflow-vs-continue.md)

## Examples

- [Example workflow composition](../content/examples/workflow_composition_example.md)
- [Example first task](../content/examples/first_task_example.md)
- [Tomorrow-morning scorecard](../content/examples/tomorrow_morning_scorecard.md)
- [Example review bundle](../content/examples/review_bundle_example.md)

## Primary project home

- **Codeberg (primary):** <https://codeberg.org/RalphWorkflow/Ralph-Workflow>
- **GitHub mirror:** <https://github.com/Ralph-Workflow/Ralph-Workflow>
