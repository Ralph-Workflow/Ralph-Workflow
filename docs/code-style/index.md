...
- **Verification**: `cd ralph-workflow && make verify` runs three stages: `make lint` (ruff check), `make typecheck` (mypy --strict), and `make test` (`uv run python -m ralph.test_suites`, which runs one 30-second-capped parallel pytest invocation over `tests/` excluding `subprocess_e2e`, with the same 30-second total budget enforced by `ralph.verify._TOTAL_TEST_BUDGET_SECONDS`)
