...
* Material, hard-to-reverse architectural decisions MUST be recorded durably
  ...
  ralph-workflow/docs/architecture/README.md (per the package-side
  architecture router page that links each ADR with its provenance).
...
RALPH-FACT: decision_record_location: ralph-workflow/docs/architecture/adr-NNNN-slug.md, MADR format. Index at ralph-workflow/docs/architecture/README.md (the package-side architecture router page that links each ADR with its provenance). Existing examples: ralph-workflow/docs/architecture/adr-0001-interrupt-architecture.md and ralph-workflow/docs/architecture/project-policy-readiness-traceability.md. A material architectural decision made without an ADR is a process defect; the agent-policy obligates the agent to record the decision before the change merges.
RALPH-FACT: quality_attribute_scenarios: (1) The 60 s combined test budget is the dominant performance scenario — verified by ralph/verify.py:_TOTAL_TEST_BUDGET_SECONDS + the _BUDGET_TRACKED_STEPS set; a regression that pushes the suite past 60 s is a fail-closed defect. (2) Determinism is mandatory for the default suite: ordinary tests inject clock / filesystem / subprocess fakes, while the explicit real-git auto-integration registry in ralph/test_suites.py is selected through the required_auto_integrate_e2e marker even when those tests also carry subprocess_e2e. The broader subprocess_e2e suite remains opt-in. (3) Async I/O is supported via pytest-asyncio (asyncio_mode=auto) for subpackage code that needs it. (4) Memory growth is bounded by the resource-lifecycle audit; long-lived accumulators must declare a cap or get flagged.
RALPH-FACT: runtime_and_deployment_views: (1) install view: `make dev` delegates to `python -m ralph.install`, which copies the checkout, runs `uv sync --extra dev` in the copied dev snapshot, and writes `rdev`; `make stable` installs the pinned stable env, and `make install` creates the system-wide manual-build snapshot. (2) runtime view: `python -m ralph` CLI; `rdev` launcher for the dev build; `ralph` launcher for the stable build. (3) CI view: `.github/workflows/verify.yml` invokes `cd ralph-workflow && make verify` on every push and pull_request event on the GitHub primary repository; `.woodpecker.yml` continues the same gate on the Codeberg mirror while maintained. `.github/workflows/cla.yml` runs the CLA checkbox check, and `.github/workflows/publish-python-package.yml` is a release-only tag pipeline that builds and publishes the package to PyPI. (4) distribution view: Homebrew formula at ralph-workflow/Formula/ralph-workflow.rb (built via `make dist-homebrew`); PyPI wheel + sdist (built via `make dist-pypi`). (5) skill distribution view: the skills-package under ralph-workflow/skills-package/ is a CommonJS npm-style distribution, packed via `npm run prepack`.
RALPH-FACT: major_data_flows: (1) CLI invocation: `python -m ralph` -> typer CLI -> ralph/agents/invoke.py -> subprocess to the configured agent CLI (Claude Code / OpenCode / Codex / Cursor / Pi) -> NDJSON stream on stdout -> ralph/parsers/* -> pydantic model -> recovery classifier -> output. (2) MCP transport: ralph/mcp/server/__main__.py -> stdio JSON-RPC -> ralph/mcp/server/_fallback_http_handler.py (local-only HTTP) -> ralph/mcp/tools/* -> workspace write/read. (3) Filesystem: any path -> FsWorkspace.write_file / read_file / append_file / edit_file -> root-normalized, path-traversal-checked, audit-tool-exec-checked. (4) Workspace awareness: WorkspaceMonitor -> bounded WorkspaceAwareness -> persisted Explore dirty queue -> coalesced ReindexWriter -> committed generation or explicit live fallback. (5) Watchdog: agent subprocess -> PostExitWatchdog (Clock-injected) -> recovery classifier -> retry or final output.
RALPH-FACT: known_risks_and_tradeoffs: (1) The 60 s combined test budget is a hard upper bound; pushing it requires a maintenance-trigger review and a recorded decision. (2) mypy --strict + ruff with the documented rule set is a maintenance burden on first-party authors; the cost is paid deliberately to keep cross-package contracts checkable. (3) The Homebrew formula is the only Ruby file; the `make formula-check` prerequisite of `make verify` syntax-checks it and fails closed when Ruby is unavailable — there is no Ruby type checker wired. (4) The legacy Rust implementation under docs/legacy-rust/ is a read-only historical pointer; reintroducing active Rust code requires a recorded decision AND updating the dependency, typecheck, and lint policies in the same workflow. (5) AI agent CLIs are subprocesses and the dominant source of flakiness; live_agy and the broader subprocess_e2e profile quarantine that flakiness, while the required_auto_integrate_e2e real-git registry is the deliberate default-profile exception.
RALPH-FACT: conformance_method: `make -C ralph-workflow verify` is the authoritative architecture-conformance gate. Its Make prerequisites enforce docs and drift checks; `ralph-workflow/ralph/verify.py:_VERIFY_STEPS` is the source of truth for ruff, mypy, the combined `make test` profile, and the remaining policy / lifecycle / architecture audit and social-proof entries in their exact execution order. The combined test selection includes the registered required_auto_integrate_e2e real-git files; there is no separate auto-integration verify step. The architecture-specific audits include audit_di_seam, audit_agent_module_state, and audit_agent_internal_paths. A red flag from any gate is a cross-component boundary violation, a forbidden import, or a state-ownership leak; the agent MUST fix the cause at the source, never by adding ignore annotations.
...
* INSPECT the component map before moving responsibilities; the per-subpackage
  seam is the documented injection point in ralph-workflow/tests/conftest.py
  (and the per-component fixture modules under ralph-workflow/tests/_support/).
...
The expected successful result is exit 0 from the authoritative verify chain
declared in ralph-workflow/ralph/verify.py. On failure, report the failing audit
