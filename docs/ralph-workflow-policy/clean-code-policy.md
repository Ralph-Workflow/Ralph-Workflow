...
RALPH-FACT: module_boundary_rule: each subpackage under `ralph/` (agents, api, checkpoint, cli, config, contrib, diagnostics, display, executor, exit_pause, files, git, guidelines, interrupt, language_detector, logging, mcp, phases, pipeline, platform, policy, pro_support, process, project_policy, prompts, recovery, runtime, skills, telemetry, testing, timeout, workspace) owns one concept and is the seam for dependency injection via the shared fixtures in `ralph-workflow/tests/conftest.py` and the per-component fixture modules under `ralph-workflow/tests/_support/`. New public subpackages require an `.. automodule::` entry in `docs/sphinx/modules.rst` (enforced by `tests/test_sphinx_modules_coverage.py`). Circular-import avoidance is delegated to lazy `PLC0415` ignores with rationale, not to a global silencer.
...
RALPH-FACT: dead_code_audit_command: `make -C ralph-workflow dead-code` runs `uv run --with vulture python -m vulture --config pyproject.toml`. Vulture's minimum-confidence floor is the single project-wide `min_confidence = 100` set in `ralph-workflow/pyproject.toml` `[tool.vulture]`; there is no separate quick-run / release-only override. Removal rather than annotation is the project default; a `# noqa`-style annotated dead symbol is a defect, not a workaround.
