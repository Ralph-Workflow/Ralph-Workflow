...
RALPH-FACT: ci_gate_integration: `make verify` runs the typechecker via the `python -m mypy ralph/` verify step (`ralph/verify.py:_VERIFY_STEPS`). `.woodpecker.yml` invokes `cd ralph-workflow && make verify` on every push and pull_request event on Codeberg and fails the build when mypy exits non-zero. `.github/workflows/cla.yml` and `.github/workflows/publish-python-package.yml` do NOT run `make verify`; the CLA workflow runs `python -m ralph.contrib.cla` only and the publish workflow is a release-only tag pipeline. `make typecheck` is a thin shortcut.
