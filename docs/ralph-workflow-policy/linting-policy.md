...
RALPH-FACT: excluded_paths: per-file-ignores in pyproject.toml `[tool.ruff.lint.per-file-ignores]` — the complete inventory of keys: `ralph/visual/**/*.py` (PLR0911, PLR0912, PLR2004, ANN401); `ralph/agents/multimodal_status.py` (PLR0911, PLR2004); `ralph/agents/delegation_capabilities.py` (PLR0911, PLR2004); `ralph/agents/vision_agent_provisioning.py` (PLR0911); `ralph/agents/builtin_spec.py` (PLR0911, PLR0912, PLC0415); `ralph/agents/support.py` (PLR0911, PLC0415); `ralph/pipeline/plumbing/smoke_multimodal.py` (PLR0911, PLR0912, PLC0415); `ralph/pipeline/plumbing/smoke_plumbing.py` (PLR0911, PLR0912, PLC0415); `ralph/mcp/multimodal/capabilities.py` (PLR0911); `ralph/testing/audit_appearance_assertion_prohibition.py` (PLR0911, PLR0912, PLR2004); `tests/**/*.py` (PLR2004, RUF043, PERF401, PLR0124, N802, TC003, ANN001, ANN201, ANN202, ANN204, ANN401, E501, PLC0415) (rationale documented inline); `ralph/cli/**/*.py` (PLC0415), `ralph/config/**/*.py` (PLC0415), `ralph/display/**/*.py` (PLC0415; lazy imports avoid circular dependencies); `ralph/**/*.py` (E501; bounded-accumulator-ok / resource-lifecycle-ok markers on assignment lines, with rationale in the audit_lint_bypass allowlist); `ralph/mcp/explore/**/*.py` (PLC0415, PLR0911, PLR0912, PLR0915, RUF012, TC003); `ralph/mcp/tools/workspace/**/*.py` (PLC0415, TC003, PLR0911, PLR0912, PLR0915); `ralph/pipeline/**/*.py` (PLC0415; lazy imports avoid the explore <-> workspace import cycle); `ralph/phases/**/*.py` (PLC0415; the `ralph.phases.__init__` deferred-import cycle break). Each block carries its rationale inline in pyproject.toml.
