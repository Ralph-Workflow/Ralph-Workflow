...
RALPH-FACT: ci_gate_integration: `make verify` runs the ruff step as the first verify step (`ralph/verify.py:_VERIFY_STEPS`, label "ruff check ralph/ tests/"). `.woodpecker.yml` invokes `cd ralph-workflow && make verify` on every push and pull_request event on Codeberg and fails the build when ruff exits non-zero. `.github/workflows/cla.yml` and `.github/workflows/publish-python-package.yml` do NOT run `make verify`; the CLA workflow runs `python -m ralph.contrib.cla` only and the publish workflow builds the distribution and publishes to PyPI on tag events.
