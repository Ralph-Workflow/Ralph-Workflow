...
RALPH-FACT: linter_per_language: Python → ruff (`make lint` → `uv run ruff check ralph/ tests/`); Ruby → `make formula-check` runs `ruby -c Formula/ralph-workflow.rb` and fails closed with an actionable gate-policy citation when Ruby is unavailable. `make verify` requires this target; CI provisions Ruby before verification. Other languages detected in the stack are recorded in the Verification section below as inapplicable.
...
RALPH-FACT: excluded_paths: per-file-ignores in pyproject.toml `[tool.ruff.lint.per-file-ignores]` — the complete inventory of keys: `ralph/visual/**/*.py` (PLR0911, PLR0912, PLR2004, ANN401); `ralph/agents/multimodal_status.py` (PLR0911, PLR2004); `ralph/agents/delegation_capabilities.py` (PLR0911, PLR2004); `ralph/agents/vision_agent_provisioning.py` (PLR0911); `ralph/agents/builtin_spec.py` (PLR0911, PLR0912, PLC0415); `ralph/agents/support.py` (PLR0911, PLC0415); `ralph/pipeline/plumbing/smoke_multimodal.py` (PLR0911, PLR0912, PLC0415); `ralph/pipeline/plumbing/smoke_plumbing.py` (PLR0911, PLR0912, PLC0415); `ralph/mcp/multimodal/capabilities.py` (PLR0911); `tests/**/*.py` (PLR2004, RUF043, PERF401, PLR0124, N802, TC003, ANN001, ANN201, ANN202, ANN204, ANN401, E501, PLC0415) (rationale documented inline); `ralph/cli/**/*.py` (PLC0415), `ralph/config/**/*.py` (PLC0415), `ralph/display/**/*.py` (PLC0415; lazy imports avoid circular dependencies); `ralph/**/*.py` (E501; bounded-accumulator-ok / resource-lifecycle-ok markers on assignment lines, with rationale in the audit_lint_bypass allowlist); `ralph/mcp/explore/**/*.py` (PLC0415, PLR0911, PLR0912, PLR0915, RUF012, TC003); `ralph/mcp/tools/workspace/**/*.py` (PLC0415, TC003, PLR0911, PLR0912, PLR0915); `ralph/pipeline/**/*.py` (PLC0415; lazy imports avoid the explore <-> workspace import cycle); `ralph/agents/invoke/_workspace.py` (PLC0415; lazy import breaks the _workspace <-> dirty_paths lifecycle coupling); `ralph/diagnostics/**/*.py` (PLC0415; lazy imports avoid the diagnostics <-> CLI/explore cycles); `ralph/phases/**/*.py` (PLC0415; the `ralph.phases.__init__` deferred-import cycle break). Each block carries its rationale inline in pyproject.toml.
...
RALPH-FACT: ci_gate_integration: `make verify` runs the ruff step as the first verify step (`ralph/verify.py:_VERIFY_STEPS`, label "ruff check ralph/ tests/"). `.github/workflows/verify.yml` invokes `cd ralph-workflow && make verify` on every push and pull_request event on the GitHub primary repository and fails the build when ruff exits non-zero. `.woodpecker.yml` continues the same gate on the Codeberg mirror while it is maintained; the CLA and publish workflows remain specialized.
...
RALPH-FACT: dead_code_detection_per_language: Python -> `make -C ralph-workflow dead-code` runs `uv run --with vulture python -m vulture --config pyproject.toml` against the ralph package and tests (vulture is in dev extras, MIT, https://github.com/jendrikseipp/vulture); Ruby -> `make -C ralph-workflow formula-check` is a Ruby syntax check (it runs `ruby -c Formula/ralph-workflow.rb`), NOT a dead-code audit; the only Ruby file is the Homebrew formula at `ralph-workflow/Formula/ralph-workflow.rb`, treated as a single build-distribution artefact. Ruby is a declared prerequisite: absence fails non-zero with the gate-policy citation rather than reporting a false pass. JavaScript and other detected languages have no dead-code audit wired because the only `.js` source is the skills-package distribution wrapper (treated as a distribution artefact). All audit findings are review-required: a finding is a defect to repair, not a threshold to raise.
...
gate the project needs). Known defect: this target is hollow on any
machine without Ruby (it exits 0 with a "skipping" message when
`ruby` is absent from `PATH`); see
`docs/ralph-workflow-policy/gate-script-policy.md`
`known_gate_corpus_defects` for the project-reality record and the
dev-cycle repair obligation.
