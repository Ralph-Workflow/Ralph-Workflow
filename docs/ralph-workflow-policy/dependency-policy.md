...
RALPH-FACT: lockfile_path: ralph-workflow/uv.lock (uv-managed; checkout installation requires `uv` >= 0.7.0 and validates this lock with `uv lock --check`, then uses `uv sync --locked --extra dev` followed by `uv sync --locked --check` in the staged snapshot). The skills-package artefact does not produce a package-lock.json because it ships zero runtime dependencies.
...
RALPH-FACT: ci_install_command: CI installs `uv` and runs `cd ralph-workflow && make verify` directly; the target resolves the checkout environment through `uv run` against `uv.lock`. `make dev` is separate: on Linux, macOS, or WSL it preflights `uv` >= 0.7.0, serializes a staged snapshot refresh, runs the locked sync sequence, validates the candidate, and writes `rdev` last. Native Windows uses the packaged install path rather than this POSIX checkout installer.
