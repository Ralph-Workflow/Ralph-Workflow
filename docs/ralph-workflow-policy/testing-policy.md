...
RALPH-FACT: fast_test_command: make -C ralph-workflow test-fast (a <=10-second, developer-feedback-only profile for changes confined to suite-routing behavior in `ralph-workflow/Makefile`, `ralph-workflow/ralph/test_suites.py`, or `tests/test_makefile_verification_workflow.py`, `tests/test_test_suites.py`, and `tests/test_test_suites_orchestration.py`; it statically selects exactly those three tests with `not subprocess_e2e and not smoke`, fails closed for a missing file, and never substitutes for `make -C ralph-workflow verify`)
RALPH-FACT: test_command_prerequisites: from the checkout, `cd ralph-workflow && make test` resolves its locked environment through `uv run`. `make dev` is optional POSIX/WSL dev-install setup: with `uv` >= 0.7.0, it stages a copied snapshot, checks the lock, runs `uv sync --locked --extra dev` and `uv sync --locked --check`, validates it, then writes `rdev` last; it does not bootstrap the current checkout in place. Native Windows uses the packaged install path.
...
RALPH-FACT: suite_time_budget: 60 seconds combined (enforced by ralph/verify.py:_TOTAL_TEST_BUDGET_SECONDS via cumulative time.monotonic() tracking across every step in ralph/verify.py:_BUDGET_TRACKED_STEPS) across `make test`, the offline fake-uv `make test-install-make-smoke`, `make test-multimodal-smoke`, and `make test-visual-smoke`. The install smoke drives the public Make target and proves candidate validation, lock contention, and rollback without a network. The combined total is the IMMUTABLE contract; per-step timeouts sit at 30 s in ralph/verify.py:_VERIFY_STEP_TIMEOUT_SECONDS. The test labels are declared in ralph/verify.py:_KNOWN_TEST_STEP_LABELS.
...
RALPH-FACT: timeout_enforcement_mechanism: three layers -- (1) per-test 1.0 s via the SIGALRM/ITIMER_REAL hookwrapper `pytest_runtest_call` in ralph-workflow/tests/conftest.py, raising TestExecutionTimeoutError; (2) per-suite wall-clock via ralph/verify_timeout.py:run_command_with_timeout, invoked as `python -m ralph.verify_timeout --suite-timeout $(PYTEST_SUITE_TIMEOUT_SECONDS)` by the Makefile suite targets and via ralph/test_suites.py for `make test`, converting a breach to exit code 124; (3) the combined 60 s budget in ralph/verify.py:_TOTAL_TEST_BUDGET_SECONDS tracked over ralph/verify.py:_BUDGET_TRACKED_STEPS, including the dedicated `make test-install-make-smoke` target rather than the ordinary `make test` marker selection. The combined budget tracker is enforced with `if` / `raise RuntimeError` (NOT `assert`) so it survives `python -O`; import-time invariants live in ralph/verify.py and are tested in ralph-workflow/tests/test_verify_invariants.py.
