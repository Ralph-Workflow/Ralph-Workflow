...
RALPH-FACT: secondary_test_frameworks: pytest-xdist (parallel workers, default 4 via PYTEST_WORKERS in ralph-workflow/Makefile: `make test` consumes the env via ralph.test_suites, and the test-unit / test-integration / test-subprocess-e2e / test-cov targets interpolate -n $(PYTEST_WORKERS) directly; the 'auto' path is capped at 4 by _MAX_PYTEST_WORKERS in ralph/test_suites.py), pytest-asyncio (asyncio_mode=auto), pytest-cov (only on test-cov target), hypothesis (property-based tests for cross-process contracts; >= 6.100 in dev extras), vulture (dead-code audit; one-shot via `make dead-code`, not part of `make test`)
