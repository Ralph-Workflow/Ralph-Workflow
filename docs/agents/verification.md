...
- **Interrupt and termination paths.** `InterruptController.force_exit` (`ralph/interrupt/controller.py`) invokes `restore_terminal` before `hard_exit` (`os._exit`); CLI handlers write the reset sequence and call `restore_terminal_modes` before delegating `SIGTERM` and `SIGHUP`.
- **Controlling-terminal and input safety.** The restore target falls back from stdout to stderr and then the controlling terminal. `terminal_understands_vt` in `ralph/display/terminal_restore.py` suppresses escape writes for unset, empty, or `TERM=dumb` terminals, but termios restoration and `termios.tcflush(..., TCIFLUSH)` remain unconditional.
- **Probe safety.** The OSC 11 background probe (`ralph/display/_terminal_bg_query.py::_probe`) publishes its pre-raw termios snapshot before `setraw`, sends its OSC query only when `terminal_understands_vt` permits it, flushes unread reply bytes (`tcflush`) on the timeout path, and restores the snapshot that existed before the probe.
