...
**Provider config policy must stay transport-specific.** In `ralph-workflow/ralph/agents/invoke.py`, Claude uses `--tools ""` to disable built-in tools and receives a strict `--mcp-config` payload containing **only** the live Ralph endpoint. User upstream MCP server definitions are extracted from supported config sources and passed to the Ralph runtime separately for proxy re-exposure; they are not included in the provider-visible `--mcp-config`. OpenCode and Codex keep their own transport-specific config merge paths, with the same strict-Ralph-only provider-visible contract. The regression seam for this policy lives in `ralph-workflow/tests/test_agents_invoke.py`.

**Upstream MCP servers are proxied behind Ralph.** In strict Ralph authority mode, user-configured upstream MCP servers are not passed through directly to provider CLIs. Instead, Ralph loads those server definitions itself and re-exposes their tools as Ralph-owned proxied aliases using the `ralph_upstream__<server_name>__<tool_name>` naming scheme. Provider-side MCP permissions are not the authority for these proxied tools; Ralph enforces capability policy before forwarding any proxied call. Only `tools` are proxied in v1; `resources` and `prompts` are out of scope. This contract applies consistently to Claude, OpenCode, and Codex integrations. See `docs/architecture/mcp-upstream-proxy.md` for the full architecture description.
